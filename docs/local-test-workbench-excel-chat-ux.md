...
DevTools 设宽度 **375px** 或真机钉钉内打开。

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| M1 | 卡片编辑器入口 | 点击「编辑草案」 | 全屏 Sheet（非 Excel 表格弹窗） |
| M2 | 子任务列表 | 查看列表 | 每条卡片显示标题、负责人、截止、已指派/未指派 badge |
| M3 | 单条编辑 | 点第 3 条 → 编辑 textarea | **输入时光标与字段始终可见**（不盲打） |
| M4 | 上一条/下一条 | 底栏切换 | 序号更新，字段内容正确 |
| M5 | 负责人搜索 | assignee 输入 1 字 | fixed portal 下拉不被键盘遮挡 |
| M6 | 保存返回 | 「保存并返回列表」 | 回到列表，修改已暂存 |
| M7 | 提交校验 | 点「提交修改（Agent 校验）」 | Agent overlay 正常，成功后关闭并刷新草案 |
| M8 | 桌面回归 | 宽度 ≥960px 再点编辑 | 仍为 Excel 表格弹窗，行为与改前一致 |

窄屏 Chat 附加：

| ID | 检查项 | 预期 |
|----|--------|------|
| M9 | 编辑按钮文案 | 显示「编辑草案」（非「编辑草案表格」） |
| M10 | 草案面板折叠 | 「收起草案面板」后消息区变高，可再展开 |

---

## 十、移动端响应式回归（2026-05-29 修复批次）

DevTools 宽度 **375px**（iPhone SE）或真机钉钉内打开。重启 `npm run dev:manager-chat` 后再测。

### 主管 · 历史任务

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F1 | KPI 卡片 | 打开任务列表 Tab | 三张 KPI 并排不溢出屏幕右缘 |
| F2 | 快速筛选 · 全部/待承接 | 点「全部」「待承接」 | 筛选条单行横向滑动，「高级筛选」卡片不裁切 |
| F3 | 快速筛选 · 不换行 | 依次点「执行中」「待您处理」「阻塞」 | 五个按钮始终同一行（可左右滑），「阻塞」不单独掉到第二行 |
| F4 | 调整分配下拉 | Tab **调整分配** → 打开任务下拉 | 选项不超出屏幕；长标题截断显示，完整 title 可 hover/长按查看 |

### 主管 · 周度 Dashboard

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F5 | 历史周提示 | 切换到非本周 | **不再**出现「您正在查看历史周…」蓝色横幅 |
| F6 | 横向滚动 | 上下滑动浏览整页 | 页面不能整体左右滑动；甘特图区域可在自身容器内横滑 |

### 主管 · 项目总览

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F7 | 新建项目弹窗 | 点「新建项目」 | 无黑色粗边框；内容居中；取消/创建按钮对齐底部 |

### 主管 · 智能规划助手

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F8 | 页面纵向滚动 | 有草案时向下滑 | 能滚到草案面板底部「编辑草案」按钮 |
| F9 | 助手表格 | 查看对话区 Markdown 表格 | 列宽正常或容器内横滑，不出现单字竖排 |
| F10 | 顶部布局 | 查看会话列表 + 对话区 | 会话列表高度受限，提示文案在窄屏隐藏 |

### 主管 · 导航

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F11 | 会议入库位置 | 打开左侧导航 | 「会议入库」在 **工作** 分组（与历史任务/Dashboard/助手同级），不在「项目」下 |

### 员工端

| ID | 检查项 | 操作 | 预期 |
|----|--------|------|------|
| F12 | 页头文案 | 切换待承接/进行中/能力画像 | 大标题随 Tab 变化；**无**顶部小字「待承接」重复 |
| F13 | 无用说明 | 打开能力画像 | **无**「主管发布后的正式子任务…」全局说明 |
| F14 | 账号安全 | 钉钉/测试登录员工账号 | 侧栏 **无**「账号安全」入口 |

---

## 十一、测试记录模板（更新）

| 日期 | 测试人 | 浏览器 | QWEN | C1–C15 | E1–E9 | M1–M10 | F1–F14 | R1–R3 | 备注 |
|------|--------|--------|------|--------|-------|--------|-------|------|
| | | Chrome / Edge | 有/无 | ☐ | ☐ | ☐ | ☐ | |

---

**建议测试顺序**：L1–L3 → C1–C5 → E1–E6（无 API 即可）→ 配置 QWEN 后 C9–E8 → **M1–M10（375px）** → R1–R3。
