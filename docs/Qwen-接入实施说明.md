...
6. **会话记忆**：`knownFacts` 通过 `list_known_facts` / `update_known_facts` 在同会话内持续累积，`conversationHistory` 参与后续轮次上下文。
7. **输出补齐**：钉钉端拿到 `draft` 后会补充结构化字段表（含 `feedbackFrequency`），避免模型自由 Markdown 漏字段。
8. **可观测**：主链路关注 `orchestrator_done`、assignment 事件与 `data/plans` 快照；`createTaskPlanningDemo` 的 `DemoGenerationMetadata`/JSONL 主要用于 demo/eval 路径。
