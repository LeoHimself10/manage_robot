...
function makeFlow(r,mode){
 ...
 const f={mode,manager:'测试主管',department:packaging?'生产与质量协作组（演示）':'研发与测试协作组（演示）',due:'2026-09-10 18:00',reviewTime:stamp(r),updated:'2026-09-08 14:10',analyses:[],tasks:[],activity:[],reopened:false};
 if(mode!=='analysis'){
...
seedFlows();
scopes.splice(0,scopes.length,'全部反馈','待我研判','待补资料','跟踪处理中','已结束');
matchScope=function(r){return state.scope==='全部反馈'||state.scope==='待我研判'&&r.review==='待研判'||state.scope==='待补资料'&&r.review==='待补资料'||state.scope==='跟踪处理中'&&flows[r.id]&&flows[r.id].mode!=='closed'||state.scope==='已结束'&&(r.review==='普通反馈'||flows[r.id]?.mode==='closed');};
...
function stageLabel(r){return flows[r.id]?modes[flows[r.id].mode].label:r.review==='普通反馈'?'普通反馈已结束':r.review==='待补资料'?'等待补充资料':'等待马荣鑫研判';}
function stageOwner(r){const f=flows[r.id];if(f?.mode==='active')return [...new Set(f.tasks.filter(t=>t.status!=='passed').map(t=>t.status==='submitted'?f.manager:t.person))].join('、');return f?modes[f.mode].owner:r.review==='待补资料'?r.person:r.review==='普通反馈'?'研判已完成':'马荣鑫';}
...
selectRecord=function(id,tab='attachments'){if(viewState.surface==='feedback'&&!['attachments','form','history','review'].includes(tab))tab='attachments';if(state.selected!==id){viewState.analysisVersion=null;viewState.taskFilter='全部任务';viewState.activityFilter='全部';viewState.auditTab='evidence';viewState.viewedPhase=null;}originalSelect(id,tab);};
...
function syncDetailNavigation(){
 ...
 document.querySelectorAll('#detail .detail-tabs [data-tab]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.tab===state.tab||b.dataset.tab==='attachments'&&raw)));
...
function detailFooter(r){
 if(viewState.surface==='feedback')return `<p>${flows[r.id]?'已关联质量事件，点击“进入质量事件”查看后续处理。':'当前为来源反馈；正式通报后才会生成质量事件入口。'}</p><div class="footer-actions"><button class="btn" data-action="oa">${icon('out')}查看 OA 原单</button>${!flows[r.id]&&r.review!=='普通反馈'&&state.tab!=='review'?'<button class="btn primary" data-tab="review">进入人工研判 '+icon('arrow')+'</button>':''}</div>`;
 return `<p>后续处理由各环节负责人完成；马荣鑫在此查看结果与进度。</p><div class="footer-actions"><button class="btn" data-action="oa">${icon('out')}查看 OA 原单</button>${state.tab!=='overview'?'<button class="btn" data-tab="overview">返回事件总览</button>':''}</div>`;
}
function goTab(tab){
 const r=currentRow();if(!r)return;
 if(viewState.surface==='feedback'&&!['attachments','form','history','review'].includes(tab))return;
...
function setSurface(surface){
 ...
 document.querySelector('.breadcrumb').innerHTML=event?'质量追踪 <span style="margin:0 8px">/</span> <button class="breadcrumb-link" data-return-feedback>全部反馈</button> <span style="margin:0 8px">/</span> 质量事件':'质量追踪 <span style="margin:0 8px">/</span> 全部反馈';
 document.title=event?'质量事件 '+(currentRow()?.eventNo||'')+' · 马荣鑫视角':'全部反馈 · 马荣鑫工作台 · 交互原型';
}
function snapshotFeedback(record){return {scope:state.scope,query:state.query,page:state.page,selected:record,tab:state.selected===record&&['attachments','form','history','review'].includes(state.tab)?state.tab:'attachments',fileKind:state.fileKind,filters:structuredClone(state.filters),filtersOpen:!$('filterPanel').hidden,scrollY:window.scrollY};}
function displayEvent(record){
 if(!flows[record])return false;
 state.selected=record;state.tab='overview';viewState.viewedPhase=null;viewState.analysisVersion=null;viewState.taskFilter='全部任务';viewState.auditTab='evidence';viewState.activityFilter='全部';
 setSurface('event');renderDetail();window.scrollTo({top:0,behavior:'instant'});return true;
}
function enterEvent(record){
 if(viewState.surface==='event')return;
 const r=rows.find(r=>r.id===record);if(!r||!flows[record])return toast('这条反馈尚未通报为质量事件。');
 const context=snapshotFeedback(record);viewState.feedbackContext=context;
 history.replaceState({maSurface:'feedback',record,feedback:context},'',feedbackUrl(record));
 history.pushState({maSurface:'event',record,feedback:context,enteredFromFeedback:true},'',eventUrl(record));
 displayEvent(record);
}
function displayFeedback(context){
 document.querySelectorAll('dialog[open]').forEach(d=>d.close());setSurface('feedback');
 const c=context||{};state.scope=c.scope||'全部反馈';state.query=c.query||'';state.page=c.page||1;state.selected=c.selected||null;state.tab=c.tab||'attachments';state.fileKind=c.fileKind||'全部';state.filters=c.filters||{approval:'',product:'',attachment:'',from:'',to:'',stage:''};
...
function initializeEventNavigation(){
 $('detailStore').insertAdjacentHTML('beforebegin',`<section id="eventWorkspace" hidden aria-label="质量事件详情"><div class="event-toolbar"><button class="btn" data-return-feedback><span style="display:inline-flex;transform:rotate(180deg)">${icon('arrow')}</span>返回全部反馈</button><span class="quiet">质量事件详情 · 查看本事件的后续处理</span></div><div id="eventDetailMount"></div></section>`);
 history.scrollRestoration='manual';
 const params=new URLSearchParams(location.search),record=params.get('record'),saved=history.state;
 if(params.get('view')==='event'&&flows[record]){viewState.feedbackContext=saved?.feedback||{selected:record,page:Math.floor(rows.findIndex(r=>r.id===record)/state.pageSize)+1};history.replaceState({...saved,maSurface:'event',record,feedback:viewState.feedbackContext},'',eventUrl(record));displayEvent(record);}
 else{setSurface('feedback');if(record&&rows.some(r=>r.id===record))selectRecord(record,'attachments');history.replaceState({maSurface:'feedback',record},'',feedbackUrl(record));}
 window.addEventListener('popstate',event=>{const route=event.state;if(route?.maSurface==='event'&&flows[route.record]){viewState.feedbackContext=route.feedback;displayEvent(route.record);}else displayFeedback(route?.feedback||viewState.feedbackContext);});
}
function phasePath(r){
 const f=flows[r.id],ordinary=r.review==='普通反馈',ix=f?modes[f.mode].index:ordinary?2:1;
 const phases=[['提交接入','attachments','员工 / OA'],['人工研判','review','马荣鑫'],['质量初析','analysis','佟成'],['任务分配','tasks','主管'],['员工承办','tasks','承办员工'],['质量终验','audit','佟成']];
...
function renderAnalysis(r){
 const f=flows[r.id];if(!f)return emptyStage('尚未进入质量初析','本条尚未通报为质量事件；不会提前生成佟成的初析结果。');
 ...
 return `${note('以下是佟成已提交的初析。每次修订独立保留，人员分配结果在“分配与承办”中查看。')}<div class="analysis-meta"><div><h3>佟成的质量初析 ${tag('V'+selected.version,isLatest?'blue':'')}${isLatest?' '+tag('当前版本','green'):''}</h3><p class="quiet" style="margin-top:6px">提交人：佟成 · ${selected.time} · 引用来源 V${r.version}</p></div><label><span class="field-label">查看初析版本</span><select id="analysisVersion">${[...f.analyses].reverse().map(a=>`<option value="${a.version}" ${a.version===selected.version?'selected':''}>V${a.version}${a.version===latest.version?' · 当前版本':' · 历史只读'} · ${a.time}</option>`).join('')}</select></label></div>${selected.reason?`<div class="change-note"><b>本次修订：</b>${esc(selected.reason)}<br>${esc(selected.sync)}</div>`:!isLatest?'<div class="change-note">正在查看历史 V1。后续修订未覆盖此版本。</div>':''}<section class="analysis-block"><h3>方向与建议</h3><div class="facts-grid">${textFact('人工确认分类',selected.category)}${textFact('建议责任部门',selected.department)}${textFact('建议总期限',selected.due)}</div></section><section class="analysis-block"><h3>分析与判断</h3>${[['问题方向',selected.direction],['来源事实摘要',selected.facts],['分析依据',selected.basis],['初步结论',selected.conclusion],['信息缺口',selected.gap]].map(([k,v])=>`<dl class="analysis-text"><dt>${k}</dt><dd>${esc(v)}</dd></dl>`).join('')}</section><section class="analysis-block"><h3>处理要求</h3><div class="long-fact" style="white-space:pre-line">${esc(selected.requirements)}</div><p class="quiet" style="margin-top:12px">初析中的部门与期限是建议；正式人员和任务期限以主管确认的分配结果为准。</p></section>`;
}
...
function renderReview(r){const f=flows[r.id],draft=viewState.reviewDrafts[r.id]||{decision:r.review==='待补资料'?'待补资料':'已通报',reason:'',saved:false};
 const intro=note('OA 来源事实保持只读；你的研判单独保存。AI 建议仅作参考，本演示没有调用 AI。');
 if(f||r.review==='普通反馈')return intro+`<section class="saved-review"><div class="section-head"><h3>马荣鑫的最终研判 ${badge(r.review)}</h3><span class="quiet">来源 V${r.version} · ${f?.reviewTime||stamp(r)} · 演示</span></div><p>${esc(r.manualConclusion||(f?'已核对原始资料，判断需要进入质量处理流程；正式通报后交佟成进行质量初析。':r.comment||'经核实属于普通反馈，本次不通报为质量事件。'))}</p>${f?`<p>已关联质量事件：<b>${r.eventNo}</b></p><button class="btn" ${viewState.surface==='event'?'data-tab="analysis"':'data-enter-event="'+r.id+'"'} style="margin-top:15px">${viewState.surface==='event'?'查看佟成初析':'进入质量事件'} ${icon('arrow')}</button>`:''}</section><section class="analysis-block"><h3>AI 原始建议</h3><p class="quiet">本原型未生成 AI 建议。后续接入时，原始建议与人工研判分别留存。</p></section>`;
 return intro+(r.review==='待补资料'?`<div class="change-note"><b>上次研判：待补资料。</b>${esc(r.manualConclusion||'需补充完整操作步骤、设备日志及现场确认。')}<br>${r.updated?'当前来源已更新为 V'+r.version+'，请重新核对。':'等待员工在 OA 补充，原始记录继续保留。'}</div>`:'')+`<section class="review-form"><div class="section-head"><h3>核对原始资料后，保存你的研判</h3><button class="text-btn" data-tab="attachments">查看 ${r.fileIds.length} 份原始附件</button></div><div class="decision-options">${[['普通反馈','保存结论，本次不进入质量事件。'],['待补资料','记录信息缺口，等待员工补充。'],['已通报','正式通报质量事件，进入佟成初析。']].map(([value,desc])=>`<label class="decision-choice"><input type="radio" name="reviewDecision" value="${value}" ${draft.decision===value?'checked':''}><span>${value==='已通报'?'通报质量异常':value}<small>${desc}</small></span></label>`).join('')}</div><label>研判结论 / 所需补充信息 <span style="color:var(--red)">*</span><textarea id="reviewReason" maxlength="2000" placeholder="写明你的判断和依据；待补资料时，明确需要补充什么。">${esc(draft.reason)}</textarea></label><p class="form-error" id="reviewError" role="alert" hidden></p><div class="form-actions"><span class="quiet" id="draftStatus">${draft.saved?'草稿已保存在本次原型会话中，尚未正式通报。':'演示操作，仅在当前页面生效；刷新后恢复样例。'}</span><button class="btn" data-save-draft>保存草稿</button><button class="btn primary" data-submit-review>提交研判（演示） ${icon('arrow')}</button></div></section>`;
}
renderDetailContent=function(r){
 if(['attachments','form','history'].includes(state.tab)){originalContent(r);$('detailContent').insertAdjacentHTML('afterbegin',`<div class="subnav source-subtabs">${[['attachments','OA 附件 '+r.fileIds.length],['form','原始表单'],['history','审批与版本']].map(([key,label])=>`<button class="chip ${state.tab===key?'active':''}" data-tab="${key}">${label}</button>`).join('')}<span class="quiet">员工原始提交，来源只读</span></div>`);return;}
 $('detailContent').innerHTML=state.tab==='overview'?renderOverview(r):state.tab==='analysis'?renderAnalysis(r):state.tab==='tasks'?renderTasks(r):state.tab==='audit'?renderAudit(r):renderReview(r);
};
function openTask(id){const r=currentRow(),f=flows[r.id];if(!f?.tasks.some(t=>t.id===id))return;viewState.taskFilter='全部任务';goTab('tasks');const d=$('task-'+id);d.open=true;d.scrollIntoView({behavior:'smooth',block:'start'});}
...
function readDraft(){const r=currentRow(),old=viewState.reviewDrafts[r.id];return viewState.reviewDrafts[r.id]={decision:document.querySelector('input[name="reviewDecision"]:checked')?.value||'已通报',reason:$('reviewReason')?.value||old?.reason||'',saved:old?.saved||false};}
function showReviewConfirmation(){const r=currentRow(),draft=readDraft();if(!draft.reason.trim()){$('reviewError').hidden=false;$('reviewError').textContent='请填写研判结论或需要补充的信息。';$('reviewReason').focus();return;}
 info('确认提交研判 · 演示',`<p>反馈：${esc(r.title)}</p><p>处理方式：<b>${draft.decision==='已通报'?'通报质量异常':draft.decision}</b></p><div class="long-fact" style="white-space:pre-wrap">${esc(draft.reason)}</div><p>${draft.decision==='已通报'?'提交后，本原型会创建关联质量事件，进入“待佟成初析”。':'提交后，本原型会保存人工结论，原始附件保持可查。'}</p><p>仅更新当前演示页面，不提交到 OA 或真实系统。</p><button class="btn primary" data-confirm-review>确认提交（演示）</button>`);$('closeInfo').textContent='返回修改';
}
function submitReview(){const r=currentRow(),draft=viewState.reviewDrafts[r.id];if(!draft?.reason.trim())return;r.manualConclusion=draft.reason;r.review=draft.decision;if(r.review==='已通报'){r.eventNo='QT-DEMO-20260908-'+r.id.replace(/\D/g,'').padStart(3,'0');flows[r.id]=makeFlow(r,'analysis');flows[r.id].reviewTime=demoNow;flows[r.id].activity=[{category:'研判',time:demoNow,person:'马荣鑫',title:'通报为质量异常',text:draft.reason,tab:'review'}];}
 $('infoDialog').close();$('closeInfo').textContent='知道了';state.scope='全部反馈';state.filters.stage='';$('stageFilter').value='';const list=filtered();state.page=Math.floor(list.findIndex(x=>x.id===r.id)/state.pageSize)+1;state.tab='review';renderList();renderDetail();toast(r.review==='已通报'?'演示：已生成质量事件，可点击入口查看后续处理。':'演示：研判结果已保存。');
}
function simulateUpdate(id){const f=flows[id];if(!f||f.mode!=='active')return;const t=f.tasks.find(t=>t.status==='returned');if(!t)return toast('本轮进度更新已展示，可重置演示后再体验。');t.priorReview=t.supervisor;t.supervisor='已补交 V2，目前等待主管重新验收。';t.status='submitted';t.progress=100;t.resubmitted='2026-09-08 15:00';t.lastUpdate=t.resubmitted;t.update='已补齐对照样本和复核签字，上传 V2 后重新提交，等待主管验收。';const e=t.evidence[0];e.versions.push({version:2,time:t.resubmitted,result:'待主管验收',content:`${e.name}\n\n交互评审演示文件 · V2\n\n所属任务：${t.title}\n提交人：${t.person}\n\n本次补充\n1. 已补齐对照样本信息。\n2. 已补齐复核记录与签字栏示例。\n3. 重新提交主管验收，当前尚未通过。\n\nV1 及其退回意见继续保留。\n所有内容仅用于交互演示。`});const pending=f.tasks.find(t=>t.status==='pending');if(pending){pending.status='active';pending.accepted='2026-09-08 14:50';pending.progress=20;pending.supervisor='尚未提交完成，暂未进入主管验收。';pending.lastUpdate='2026-09-08 14:55';pending.update='已确认承接，正在整理抽样范围与核对清单。';f.activity.push({category:'承办',time:pending.accepted,person:pending.person,title:'承接任务：'+pending.title,text:'已确认承接并开始执行。',taskId:pending.id});}
...
document.addEventListener('click',e=>{
 ...
 if(Object.hasOwn(b.dataset,'saveDraft')){const d=readDraft();d.saved=true;$('draftStatus').textContent='草稿已保存在本次原型会话中，尚未正式通报。';toast('演示草稿已保存，未创建质量事件。');}
 if(Object.hasOwn(b.dataset,'submitReview'))showReviewConfirmation();
 if(Object.hasOwn(b.dataset,'confirmReview'))submitReview();
...
document.addEventListener('change',e=>{if(e.target.id==='analysisVersion'){viewState.analysisVersion=Number(e.target.value);renderDetailContent(currentRow());}if(e.target.id==='evidenceVersion'){const v=viewState.evidence;openEvidence(v.taskId,v.evidenceId,e.target.value);}if(e.target.name==='reviewDecision')readDraft();});
document.addEventListener('input',e=>{if(e.target.id==='reviewReason'){const d=readDraft();d.saved=false;if($('reviewError'))$('reviewError').hidden=true;}});
document.querySelector('h1').textContent='全部反馈';
