...
const handlingLabels = {QUALITY_ANOMALY:'质量异常', NEEDS_INFO:'待补资料', ORDINARY:'普通反馈'};
...
const manualKeys = ['handling','primary','secondary','risk','description','conclusion','changeReason','missing'];
const comparedKeys = ['handling','primary','secondary','risk','conclusion'];
const keyLabels = {handling:'处理方式',primary:'一级分类',secondary:'二级分类',risk:'风险等级',conclusion:'研判结论'};
function newAssessment(r){return {snapshots:[],selectedAI:null,running:false,error:'',adoption:'MANUAL',adoptedId:null,saved:[],dirty:true,finalized:false,draft:{handling:'',primary:'',secondary:'',risk:'',description:r.what||r.title,conclusion:'',changeReason:'',missing:''}};}
...
function confirmIntake(id){
 ...
 info('确认进入质量事件',`<p class="confirm-lead">将这条事件加入你的<strong>待我研判</strong>？</p><div class="confirm-record"><b>${esc(r.title)}</b><p>OA ${r.no} · 来源 V${r.version}</p><p>${esc(r.person)} · ${r.date} · ${r.fileIds.length} 份原始附件</p></div><p>确认后，由你运行 AI 辅助研判并作出最终判断。此时尚未通报质量异常，也不会推送佟成初析。</p><button class="btn primary" data-confirm-intake="${id}">确认加入待我研判 ${icon('arrow')}</button>`);
...
function updateWorkspaceHeading(){
 ...
 document.querySelector('.event-toolbar').innerHTML=`<button class="btn" data-return-feedback>← 返回${esc(scope)}</button><span class="quiet">${pending?'已确认选入 · 保存研判后再确认处理去向':'质量事件 · 持续跟踪初析、分配、承办与验收'}</span>`;
...
function renderAssessment(r){
 ...
 const intro=`<div class="assessment-intro"><div><b>先 AI 辅助，再由你确认</b><p>已选入待我研判 · 来源 V${r.version} · ${admissions[r.id].at}。${a.finalized?'已保存处理结论。':'确认通报质量异常后，才会推送佟成初析。'}</p></div><button class="btn" data-tab="attachments">${icon('clip')}查看原始附件 ${r.fileIds.length}</button></div>`;
...
function dispositionLabel(h){return h==='QUALITY_ANOMALY'?'确认并推送质量初析':h==='ORDINARY'?'确认普通反馈并结束':h==='NEEDS_INFO'?'确认待补资料':'保存后确认处理去向';}
function dispositionText(d){return d.handling==='QUALITY_ANOMALY'?'确认后正式通报质量事件，从待我研判移至跟踪处理中，推送佟成进行质量初析。':d.handling==='ORDINARY'?'确认后记录为普通反馈并结束，原始资料和人工结论继续保留。':d.handling==='NEEDS_INFO'?'确认后移入待补资料，保留补充要求；新的来源版本到达后重新研判。':'先选定处理方式，再保存人工研判。';}
function renderHumanHistory(a){return a.saved.length?`<details class="human-history"><summary>人工研判版本记录 · ${a.saved.length} 版</summary>${[...a.saved].reverse().map(s=>`<article><b>人工 V${s.version} · ${handlingLabels[s.draft.handling]}</b><small>${s.by} · ${s.at} · 来源 V${s.sourceVersion}${s.aiId?' · 引用 '+s.aiId:' · 人工填写'}</small><p>${esc(s.draft.conclusion)}</p>${s.draft.changeReason?`<p>原因：${esc(s.draft.changeReason)}</p>`:''}</article>`).join('')}</details>`:'';}
...
function updateAssessmentDiff(){
 ...
 $('missingField').hidden=a.draft.handling!=='NEEDS_INFO';
 $('dispositionText').textContent=dispositionText(a.draft);
 const button=document.querySelector('[data-confirm-disposition]');
 if(button){button.disabled=!a.saved.length||a.dirty;button.innerHTML=dispositionLabel(a.draft.handling)+' '+icon('arrow');}
 $('assessmentSaveStatus').textContent=a.dirty?'当前内容尚未保存；保存不会推送佟成初析。':'人工 V'+a.saved.at(-1)?.version+' 已保存，尚未确认处理去向。';
...
function applySuggestion(mode){
 ...
 if(mode!=='MANUAL'){for(const k of ['handling','primary','secondary','risk','description','conclusion','missing'])a.draft[k]=s[k];a.draft.changeReason='';}
...
function validateAssessment(a){
 const d=a.draft;
 if(!d.handling||!d.primary||!d.secondary||!d.risk||!d.description.trim()||!d.conclusion.trim())return '请补全处理方式、两级分类、风险、问题描述与研判结论。';
 if((a.adoption!=='DIRECT'||assessmentDiff(a).length)&&!d.changeReason.trim())return '请填写人工判断 / 修正原因，说明与 AI 建议不同的依据。';
 if(d.handling==='NEEDS_INFO'&&!d.missing.trim())return '请明确填写需要补充的信息。';
 return '';
}
...
function confirmDisposition(){
 ...
 info(dispositionLabel(s.draft.handling)+' · 演示',`<div class="confirm-record"><b>${esc(r.title)}</b><p>OA ${r.no} · 来源 V${r.version} · 人工 V${s.version}</p><p>处理方式：${handlingLabels[s.draft.handling]} · 风险：${riskLabels[s.draft.risk]}</p><p>分类：${esc(s.draft.primary)} / ${esc(s.draft.secondary)}</p></div><p class="long-fact">${esc(s.draft.conclusion)}</p><p>${dispositionText(s.draft)}</p><p class="quiet">仅更新本次原型会话，不向真实系统发送通知或提交数据。</p><button class="btn primary" data-submit-disposition="${r.id}" data-human-version="${s.version}">${dispositionLabel(s.draft.handling)}</button>`);
...
function commitDisposition(id,version){
 ...
 r.manualConclusion=s.draft.conclusion;r.review=s.draft.handling==='QUALITY_ANOMALY'?'已通报':s.draft.handling==='ORDINARY'?'普通反馈':'待补资料';a.finalized=true;
 if(r.review==='已通报'){
...
function reopenAssessment(){
 ...
 a.draft={...a.draft,conclusion:'',changeReason:''};renderTabs();renderAssessmentBody();
