...
function buildFlowExample(r,scene='closed'){
 ...
 f.activity=[{category:'研判',time:f.reviewTime,person:'马荣鑫',title:'完成分类与风险研判并推送初析',text:'分类：软件与数据功能 / 数据、报告与测量；风险：中。',tab:'review'},...f.analyses.map(a=>({category:'初析',time:a.time,person:'佟成',title:'提交质量初析 V'+a.version,text:a.version===1?'形成分析方向、信息缺口及建议责任部门。':a.reason,tab:'analysis'})),{category:'分配',time:f.handoffAt,person:'佟成',title:'确认唯一主责部门并推送对应主管',text:'初析 V2、必须成果清单及建议期限已推送软件研发部主管程远的原任务规划入口。',tab:'tasks'},{category:'分配',time:f.assignedAt,person:f.manager,title:'部门主管在原任务系统发布 4 项任务',text:'唯一主责部门：软件研发部；程远主管确认 4 位部门员工的承办事项、交付物与期限。',tab:'tasks'}];
 ...
 if(['quality','returned','closed'].includes(scene))f.activity.push({category:'验收',time:'2026-09-08 13:20',person:f.manager,title:'部门主管整体验收并提交证据包',text:'4 项任务通过本部门主管逐项验收，主管核对整体验收要求后提交佟成质量终验。',tab:'audit'});
