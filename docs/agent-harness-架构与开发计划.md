...
- **当前 Demo 实现**：钉钉主链路使用 `src/agent/orchestrator.ts`（ReAct + tool calling），`src/agent/demo/pipeline.ts` 保留给 CLI demo/eval。提示词版本见 `src/agent/demo/qwen-prompt.ts`（当前 `orchestrator-agent-v5.14`），关键词分类 / 模板骨架 WBS / 语义默认补全已移除。主链路草案输出已采用**最终 JSON 直出 `draft`**，不再依赖 `save_draft` 工具回合。
