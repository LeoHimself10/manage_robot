...
describe("renderQualityTrackingPage", () => {
  ...
  it("shows the five action-oriented buckets for the isolated test supervisor", () => {
    ...
    try {
      ...
      expect(html).not.toContain("去原主管任务验收");
      expect(html).toContain("renderManagerInlineReview(view, item, body)");
      expect(html).toContain("/api/workbench/manager/quality-review");
      expect(html).toContain("确认退回该任务");
      expect(html).toContain("expectedVersion: item.reviewNodeVersion");
