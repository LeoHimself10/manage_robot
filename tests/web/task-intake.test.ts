  ...
  it("previews then publishes when every row has an assignee (non-portfolio manager)", async () => {
    ...
    const commitReq = stubReq({
      ...
      body: JSON.stringify({
        ...
        parentDescription: "本周注册申报整体推进",
        rows: [
          { itemId: "ti_1", selected: true, title: "整理资料", objective: "", deliverables: "资料整理报告", completionCriteria: "资料已归档", actions: "", dependsOn: "", assigneeUserId: "u-a" },
          { itemId: "ti_2", selected: true, title: "提交申请", objective: "", deliverables: "申请材料", completionCriteria: "申请已受理", actions: "", dependsOn: "", assigneeUserId: "u-b" },
        ],
      }),
    });
  ...
  it("stages a draft to the main thread when an assignee is missing", async () => {
    ...
    const commitReq = stubReq({
      ...
      body: JSON.stringify({
        ...
        rows: [
          { itemId: "ti_1", selected: true, title: "整理资料", objective: "", deliverables: "资料整理报告", completionCriteria: "资料已归档", actions: "", dependsOn: "", assigneeUserId: "u-a" },
          { itemId: "ti_2", selected: true, title: "提交申请", objective: "", deliverables: "申请材料", completionCriteria: "申请已受理", actions: "", dependsOn: "", assigneeUserId: "" },
        ],
      }),
    });
    const commitRes = stubRes();
