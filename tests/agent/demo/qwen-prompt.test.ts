...
describe("buildQwenPlannerSystemPrompt", () => {
  it("v5.23.2: JSON contract, modes, no PREPARE mode", () => {
    const sys = buildQwenPlannerSystemPrompt();
    expect(sys).toContain("orchestrator-agent-v5.23.2");
    ...
    expect(sys.length).toBeLessThanOrEqual(6500);
  });

  it("v5.23.2: managerFollowup injects FOLLOWUP with continuous step ③", () => {
  ...
  it("v5.23.2: tools and key disciplines", () => {
    const sys = buildQwenPlannerSystemPrompt();
...
describe("buildQwenPlannerSystemPrompt employee profile", () => {
  it("requires get_task_detail for overall task background questions", () => {
    ...
    expect(sys).toContain("orchestrator-agent-v5.23.2-employee");
