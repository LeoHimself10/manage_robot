...
describe("buildQwenPlannerSystemPrompt", () => {
  it("v5.18: no switch_back_task; publish/draft/candidate-pool + history discipline prompt", () => {
    const sys = buildQwenPlannerSystemPrompt();
    expect(sys).toContain("orchestrator-agent-v5.18.0");
    ...
    expect(sys).not.toContain("switch_back_task");
    expect(sys).toContain("未发布的旧草案不支持");
...
describe("buildQwenPlannerSystemPrompt employee profile", () => {
  it("requires get_task_detail for overall task background questions", () => {
    ...
    expect(sys).toContain("orchestrator-agent-v5.18.0-employee");
