...
export function createQualityEventQuery(dbPath = resolveWorkbenchSqlitePath()) {
  ...
  function loadEvent(eventId: string): DatabaseRow | undefined {
    return db.prepare("SELECT * FROM quality_events WHERE id=? AND COALESCE(is_test,0)=0 AND deleted_at IS NULL").get(eventId) as DatabaseRow | undefined;
  }
  ...
  function listEvents(input: { viewerUserId: string }): QualityEventListItem[] {
    ...
    const rows = db.prepare(`
      ...
      ${handoffJoin}
      WHERE e.deleted_at IS NULL AND COALESCE(e.is_test,0)=0 AND (
