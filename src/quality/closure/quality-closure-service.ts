...
export function createQualityClosureService(deps?: { dbPath?: string; now?: () => string; id?: () => string }) {
  ...
  function reopenFormal(nodeId: string, reason: string): void {
    ...
    createWorkbenchFormalTaskStore().updateSubtaskStatus({ subtaskId: String(link.subtask_id), actorUserId: node.assigneeUserId, action: "progress", progressStatus: "IN_PROGRESS", note: reason });
  }
