...
function safePerspective(value: unknown): QualityPerspective | null {
  ...
  return ["aftersales", "quality_management", "manager", "dashboard"].includes(perspective)
...
export function createQualityEventPerspectiveProjector(
  ...
  function originalAssessment(eventId: string) {
    ...
    if (tableExists(db, "quality_analysis_versions")) {
      const row = db.prepare(`
        SELECT content_json FROM quality_analysis_versions
        WHERE event_id=? ORDER BY analysis_version DESC,created_at DESC LIMIT 1
      `).get(eventId) as DatabaseRow | undefined;
      ai = row ? parseObject(row.content_json) : null;
    }
    const sourceKeys = (db.prepare("SELECT source_key FROM quality_event_source_links WHERE event_id=?")
      .all(eventId) as DatabaseRow[]).map((row) => String(row.source_key));
    const ruleReasons: string[] = [];
    if (sourceKeys.length > 0 && tableExists(db, "quality_candidates")) {
      const sourceKeySet = new Set(sourceKeys);
      for (const row of db.prepare("SELECT source_keys_json,explanation_json FROM quality_candidates ORDER BY detected_at DESC").all() as DatabaseRow[]) {
        if (!parseArray(row.source_keys_json).some((key) => sourceKeySet.has(String(key)))) continue;
        const explanation = parseObject(row.explanation_json);
        const decision = explanation.decision && typeof explanation.decision === "object"
          ? explanation.decision as Record<string, unknown>
          : {};
        for (const trigger of Array.isArray(decision.triggers) ? decision.triggers : []) {
          const item = trigger as Record<string, unknown>;
          const label = nullable(item.label);
          if (label && !ruleReasons.includes(label)) ruleReasons.push(label);
        }
      }
    }
    const reasons = Array.isArray(ai?.reasons)
      ? ai!.reasons.map((item) => String(item)).slice(0, 20)
      : ruleReasons.slice(0, 20);
    return {
      available: ai != null,
      summary: nullable(ai?.summary)
        ?? (ai ? "已保存 AI 原始研判建议" : "当前事件未保存独立 AI 原始建议；规则提示仅供人工核对。"),
      recommendedDecision: ai?.recommendedDecision == null
        ? "建议暂不可用"
        : qualityDecisionLabel(ai.recommendedDecision),
      suggestedRisk: nullable(ai?.suggestedRisk) ?? "风险待人工确认",
      evidenceStrength: ["证据较强", "一般", "证据不足"].includes(String(ai?.evidenceStrength ?? ""))
        ? String(ai!.evidenceStrength)
        : "证据情况暂不可用",
      reasons,
      missingInformation: Array.isArray(ai?.missingFields)
        ? ai!.missingFields.map((item) => String(item)).slice(0, 20)
        : [],
    };
  }

  function finalAssessment(eventId: string) {
    if (!tableExists(db, "quality_source_reviews")) return [];
    return (db.prepare(`
      SELECT r.status,r.note,r.decided_by,r.decided_at
      FROM quality_source_reviews r
      JOIN quality_event_source_links l ON l.source_key=r.source_key
      WHERE l.event_id=? ORDER BY r.decided_at
    `).all(eventId) as DatabaseRow[]).map((row) => ({
      conclusion: qualityDecisionLabel(row.status),
      note: nullable(row.note) ?? "无补充说明",
      reviewerName: displayName(row.decided_by),
      decidedAt: String(row.decided_at),
    }));
  }

  function initialAnalysis(eventId: string, perspective: QualityPerspective) {
    if (!tableExists(db, "quality_initial_analysis_versions")) return { latest: null, versions: [] };
    const rows = db.prepare(`
    ...
    const versions = allowed.map((row) => ({
      ...
      versionLabel: `V${Number(row.version)}`,
      statusLabel: String(row.status) === "COMPLETED" ? "已完成" : "草稿",
