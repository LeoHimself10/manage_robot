...
function adminSwitchClientJs(): string {
  ...
  open.addEventListener('click',function(){dialog.showModal();void load();});kind.addEventListener('change',function(){void load();});query.addEventListener('input',function(){clearTimeout(timer);timer=setTimeout(load,220);});
})();</script>`;
}
