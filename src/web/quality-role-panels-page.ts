...
function buildQualityRolePanelsClientScript(): string {
  ...
  function renderBranch(view){var mount=document.getElementById('qualityPanelBranch');var actions=document.getElementById('qualityManagerActions');actions.hidden=view.readonly||view.perspective!=='manager'||(view.allowedActions.indexOf('accept')<0&&view.allowedActions.indexOf('reject')<0);clear(mount);if(!view.branch.length){mount.appendChild(el('div','qt-empty','尚未形成责任分支。'));return;}view.branch.forEach(function(item){var card=el('article','qt-tree-node');card.style.marginLeft=Math.min(48,(item.parentActionRef?20:0))+'px';var top=el('div','qt-event-row-head');top.appendChild(el('strong','',item.assigneeName+' · '+item.assigneeTypeLabel));top.appendChild(el('span','qt-status',item.statusLabel));card.appendChild(top);card.appendChild(el('div','qt-row-meta',item.departmentName+' · 期限 '+formatTime(item.dueAt)));card.appendChild(el('p','qt-row-desc',item.requirement));mount.appendChild(card);});}
