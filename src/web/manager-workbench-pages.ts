...
export const QUALITY_TASK_REPLAN_MESSAGE =
  "请作为任务拆解机器人，结合质量事件背景和质量初析，把当前宽泛成果重新规划为可执行、可承接、可验收的完整任务方案。必须覆盖已选的全部成果，但不要把一个成果简单等同于一个任务；每个必选成果的原始名称必须至少原样保留在一项任务的交付物中，以保持质量覆盖映射。围绕复现、原因定位、措施实施和效果验证补齐必要步骤。本轮先完成任务结构拆解；拆出的每一项任务都必须同时生成非空的交付物和完成标准，并补全截止时间和前后依赖，不允许只生成标题和目标。完成标准仅描述员工可交付的成果和可验证条件，不得把主管审批通过、质量终验或关闭质量事件作为员工提交完成的前置条件；这些是提交后的独立验收环节。不要写入、沿用或调整正式负责人，拆解完成后由主管在右侧逐项配置负责人。只生成待确认草案，不要发放。";
