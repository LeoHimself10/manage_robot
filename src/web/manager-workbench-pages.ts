...
export const QUALITY_TASK_REPLAN_MESSAGE =
  "[QUALITY_FULL_PLAN] 请作为任务拆解机器人，结合质量事件背景和质量初析，把当前宽泛成果重新规划为可执行、可承接、可验收的完整任务方案。必须覆盖已选的全部成果，但不要把一个成果简单等同于一个任务；根据初析要求编排实际工作，交付物使用清晰名称，不要求逐字匹配成果名称。围绕复现、原因定位、措施实施和效果验证补齐必要步骤。本轮必须一次返回覆盖全部已选成果的完整任务方案，不得只生成回收、登记等第一步就结束。输出前逐项核对必须成果清单，缺项时补全任务；可以合并任务，但不能省略成果。优先使用完整草案替换，不把只修改一行当作整套方案。 本轮先完成任务结构拆解；若存在待规划占位项，请删除占位项并创建实际任务。拆出的每一项任务都必须同时生成非空的交付物和完成标准，并补全截止时间和前后依赖，不允许只生成标题和目标。完成标准仅描述员工可交付的成果和可验证条件，不得把主管审批通过、质量终验或关闭质量事件作为员工提交完成的前置条件；这些是提交后的独立验收环节。不要写入、沿用或调整正式负责人，方案生成后由主管在右侧配置负责人、期限和完成标准，随后直接发放，无需成果映射确认。只生成待确认草案，不要发放。";
...
function buildManagerQualityClientJs(): string {
  ...
  function askQualityReason(title) {
    return new Promise(function(resolve) {
      var box=el('dialog','mq-dialog');box.setAttribute('aria-label',title);
      var form=el('form','');var heading=el('h3','',title);var label=el('label','','原因');
      var input=el('textarea','');input.required=true;input.rows=4;input.style.cssText='width:100%;box-sizing:border-box;margin:12px 0;line-height:1.7';input.setAttribute('aria-label','退回或驳回原因');label.appendChild(input);
      var actions=el('div','mq-actions');var cancel=el('button','btn btn-secondary','取消');cancel.type='button';var submit=el('button','btn btn-primary','确认退回');submit.type='submit';actions.append(cancel,submit);form.append(heading,label,actions);box.appendChild(form);document.body.appendChild(box);
      function finish(value){box.close();box.remove();resolve(value);}
      cancel.onclick=function(){finish(null);};box.addEventListener('cancel',function(e){e.preventDefault();finish(null);});
      form.onsubmit=function(e){e.preventDefault();var value=input.value.trim();if(!value){input.setCustomValidity('请填写原因');input.reportValidity();return;}finish(value);};input.oninput=function(){input.setCustomValidity('');};box.showModal();input.focus();
    });
  }
  async function action(node, kind) {
    if (kind === 'reject') { var reason = await askQualityReason('请输入驳回原因'); if (!reason) return; await post('/api/workbench/manager/quality-nodes/'+encodeURIComponent(node.nodeId)+'/reject',{expectedVersion:node.version,requestId:uuid(),reason:reason}); }
      ...
      <section class="quality-planning-enhancer" id="qualityPlanningEnhancer" hidden aria-labelledby="qualityPlanningEnhancerTitle">
        <div class="quality-planning-enhancer-head">
          ...
          <div class="quality-planning-copy">
            <h2 id="qualityPlanningEnhancerTitle">生成任务方案，配置后直接发放</h2>
            <p>生成或编辑任务，配置负责人、期限和完成标准后即可发放。</p>
          </div>
...
<div class="wb-modal-overlay planning-person-modal" id="planningPersonModalOverlay" role="dialog" aria-modal="true" aria-labelledby="planningPersonModalTitle">
  <div class="wb-modal" role="document">
    ...
    scriptHtml: `<script src="/static/workbench-draft-grid.js?v=draft-dependencies-20260918-r19"></script>
<script>
(function () {
  ${buildWorkbenchViewSwitchClientJs()}
  wbBindViewSwitchLink('navMyTasks', 'employee', '/workbench/employee?view=new');
  ...
  function renderQualityPanel(summary, qualityChecks) {
    ...
    return [
      renderQualityPanelSection('tasks', '1 · 任务方案', qualityChecks[0], qualityChecks[0] ? '已有 ' + summary.count + ' 项任务' : '请先生成或编辑任务', renderQualityTaskLinks(summary, 'tasks'), false),
      renderQualityPanelSection('people', '2 · 负责人配置', qualityChecks[1], !tasksReady ? '请先生成任务方案' : (qualityChecks[1] ? summary.count + '/' + summary.count + ' 已配置' : '还缺 ' + summary.unassigned + ' 项'), renderQualityAssigneeBody(summary), !tasksReady),
  ...
  function renderPlanningTaskCard(task, idx, isQuality, assignmentLocked) {
    ...
    var assignee = assignmentLocked
      ? '<span class="planning-task-assignee"><span class="planning-task-avatar is-pending">—</span><strong>生成任务后配置</strong></span>'
      : (assigned
  ...
  function qualityPlanningPresentation(summary) {
    ...
    var countLabel = generated ? summary.count + ' 项任务草案' : '尚未形成任务方案';
    if (!generated) return {state:'待规划', countLabel:countLabel, hint:'请生成或手工编排任务方案，再确认任务内容。'};
    if (!confirmed) return {state:'待规划', countLabel:countLabel, hint:'请先生成或编辑实际任务。'};
    if (!summary.assigneeStepReady) return {state:'待配负责人', countLabel:countLabel, hint:'请为每项任务配置负责人，补齐期限和完成标准后即可发放。'};
  ...
  function updateQualityPlanningEnhancer() {
    ...
    if (status) {
      status.textContent = qualityPlanningInFlight
        ...
        : (hasStructuredTasks
          ? ('当前已有 ' + taskCount + ' 项任务草案，配置完成后可直接发放。')
          : '先生成任务方案，或点击“编辑草案”填写任务。');
    }
  ...
  function maybeOpenDraftEditorFromUrl() {
    ...
    if (cachedDraftSummary && cachedDraftSummary.qualityContext) return;
