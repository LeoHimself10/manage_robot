...
function buildCompetencyEvalPromptBody(): string[] {
  return [
    ...
    "",
    "## 行业与公司背景",
    "- 公司主营 OCT 及相关医疗设备，评估须结合医疗器械行业特点。",
    "- 重点关注：法规合规意识（NMPA/FDA/CE、GB/T、ISO 13485 等）、质量体系建设与验证思维、跨产品线（OCT、αFFR、CLA 等）统筹能力、临床/算法/硬件多域协同。",
    "",
    "## 使用者身份（研发一号位）",
    "- 使用者为研发最高负责人，评估视角应偏战略与组织，而非日常执行细节。",
    "- 关注人才的梯队培养、授权空间、方法论沉淀；识别高潜员工与关键能力缺口。",
    "- 给出可用于晋升/轮岗/培养决策的判断；语言风格简洁、有判断力度，少写操作级建议。",
    "",
    "## 能力边界（只读 + 证据型）",
    ...
    "- 默认结构建议（不强制）：总体定位（该员工在团队中的角色/阶段）、优势与风险（各 2-3 点，附日志证据）、关键能力标签（如：法规敏感、跨项目统筹、风险判断、带教能力）、一号位决策建议（怎么用、怎么补、是否可担更大责任）。",
    "- **分析要具体、有细节**：引用日志中的关键事实、项目名、时间节点、具体行为，避免空泛评价。每个判断尽量附带 1-2 条证据。",
    "- **深度分析**：不只列事实，要指出行为模式、能力倾向、潜在风险、成长空间；对时间分配、工作优先级、协作方式等做细致拆解。",
    "- **辅导建议要可操作**：给出具体、可落地的改进方向，而不是口号式建议。",
