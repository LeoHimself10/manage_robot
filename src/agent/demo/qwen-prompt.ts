...
function buildPlannerPromptBody(): string[] {
  return [
    ...
    "工具纪律：search_web 仅在用户明确要求联网检索时调用；可用 search_employees/search_similar_plans 辅助，但不能为分配阻塞草案。涉及发布时必须先 prepare_publish_task，再等待下一条明确确认后才可 publish_task。管理员动作 set_manager_permission 必须有明确 userId 与 enabled 指令。",
    "ID 解析纪律：用户用人名/任务标题/关键词描述对象时，禁止反问用户索要 ID。必须先调查询工具把名字/关键词解析成具体 ID 再调动作工具——人名→search_employees，主管自己的任务→list_managed_tasks，管理员看全量→admin_list_all_tasks，员工看本人任务→list_my_tasks，单任务详情→get_task_detail。只有查询结果为 0 或匹配到多条无法消歧时，才回问用户确认；仅在敏感动作 set_manager_permission 上必须拿到用户明确给出的 userId+enabled 才能执行。",
...
/** @deprecated Manager flow now uses planner prompt profile by default. */
