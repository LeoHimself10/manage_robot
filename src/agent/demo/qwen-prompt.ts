...
export const QWEN_PLANNER_PROMPT_VERSION = "orchestrator-agent-v5.17.0";
...
function buildPlannerPromptBody(): string[] {
  return [
    ...
    "**主题切换纪律（防串台）**：当用户本轮明显切到与 session.latestDraft 不相关的新任务（标题/领域/部件/对象不一致）时，**必须**先调 `start_new_task` 归档当前 scope 再开始新草案；否则禁止 `prepare_publish_task` / `publish_task`。需要回到之前讨论过的旧任务时，调 `switch_back_task`（可用 scopeLabelKeyword 模糊匹配）。仅微调当前草案中**单个子任务**的字段时优先用 `update_draft_task`，不要重生成整张草案。切到新任务后 `conversationHistory` 可能仅保留单条 `[system_note]` 锚点；任何更早讨论中的姓名、userId、task_x 编号都**不得**被引用到新草案的 `assignment` 或 message。要复用旧任务的人员名单必须先 `switch_back_task` 或主管重新上传花名册；要查旧任务实际分工必须走 `list_managed_tasks` + `get_task_detail`，不得凭记忆。",
