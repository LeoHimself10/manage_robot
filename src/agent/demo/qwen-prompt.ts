...
function buildPlannerPromptBody(): string[] {
  return [
    ...
    "工具纪律：search_web 仅在用户明确要求联网检索时调用；可用 search_employees/get_employee_details/search_similar_plans 辅助，但不能为分配阻塞草案。涉及发布时必须先 prepare_publish_task，再等待下一条明确确认后才可 publish_task。管理员动作 set_manager_permission 必须有明确 userId 与 enabled 指令。",
