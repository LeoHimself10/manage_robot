...
(() => {
  ...
  const originalViews=[
    ['主管工作台','模拟主管 · 分配与验收','/workbench/quality?perspective=manager'],
    ['员工工作台','模拟员工1 · 承接与执行','/workbench/quality?perspective=employee']
  ];
  ...
  container.innerHTML=`<button type="button" id="viewSwitchButton" class="view-switch-trigger" aria-expanded="false" aria-controls="viewSwitchPanel">切换视角<svg viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="m4 6 4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button>
    <div id="viewSwitchPanel" class="view-switch-panel" hidden>
      <h3>质量工作台</h3><nav class="view-switch-list" aria-label="质量工作台视角">
        <button type="button" class="view-switch-link" id="viewSwitchCurrent" aria-current="page"><span><strong>质量主管</strong><small>员工 · 质量管理能力已开通</small></span><span class="view-switch-current">当前视角</span></button>
        ${link('客服主管','主管 · 反馈研判','http://127.0.0.1:8808/ma-workbench/')}
  ...
  container.addEventListener('click',event=>{
    ...
    readEditor();readFinal();persist();close();
  });
