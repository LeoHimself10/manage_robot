...
const modes={analysis:{label:'待质量主管初析',index:2,owner:'质量主管',next:'完成质量初析，明确处理要求和建议责任部门。'},assignment:{label:'待主管分配',index:3,owner:'测试主管',next:'主管在原任务系统确认任务、承办人和期限。'},active:{label:'员工处理中',index:4,owner:'测试员工2、测试员工3',next:'员工2补充复测证据；员工3确认承接抽样复核任务。'},quality:{label:'待质量终验',index:5,owner:'质量主管',next:'主管验收已通过，等待质量主管核对证据并填写终验结论。'},closed:{label:'已关闭',index:6,owner:'流程已结束',next:'终验结论、任务结果及历史证据均可追溯。'}};
...
function makeFlow(r,mode){
 ...
 f.activity=[{category:'研判',time:f.reviewTime,person:'客服主管',title:'通报为质量异常',text:'已保存人工研判，关联质量事件 '+r.eventNo+'。',tab:'review'}];
 if(f.analyses.length)f.activity.push({category:'初析',time:'2026-09-07 09:20',person:'质量主管',title:'完成质量初析 V1',text:'提出问题方向、建议责任部门和处理要求。',tab:'analysis'});
 if(f.analyses.length>1)f.activity.push({category:'初析',time:'2026-09-07 11:00',person:'质量主管',title:'发布初析 V2',text:'补充复测证据要求；V1 继续保留。',tab:'analysis'});
 ...
 if(mode==='quality')f.activity.push({category:'验收',time:'2026-09-08 14:25',person:f.manager,title:'全部任务通过主管验收',text:'已汇总证据，送交质量主管进行质量终验。',tab:'audit'});
 if(mode==='closed'){f.closedAt='2026-09-08 14:40';f.final='处理措施和验证记录已经核对，主管验收通过；质量主管完成质量终验并关闭事件。';f.activity.push({category:'终验',time:f.closedAt,person:'质量主管',title:'质量终验通过，事件关闭',text:f.final,tab:'audit'});}
 return f;
}
function seedFlows(){
...
function stageLabel(r){if(!admissions[r.id])return '尚未选入研判';return flows[r.id]?modes[flows[r.id].mode].label:r.review==='普通反馈'?'普通反馈已结束':r.review==='待补资料'?'等待补充资料':'等待客服主管研判';}
function stageOwner(r){if(!admissions[r.id])return '查看详情后确认选入';const f=flows[r.id];if(f?.mode==='active')return [...new Set(f.tasks.filter(t=>t.status!=='passed').map(t=>t.status==='submitted'?f.manager:t.person))].join('、');return f?modes[f.mode].owner:r.review==='待补资料'?r.person:r.review==='普通反馈'?'研判已完成':'客服主管';}
...
renderList=function(){
 ...
 $('listContent').innerHTML=`<div class="table-wrap"><table aria-label="客服主管反馈与处理进度列表"><colgroup><col style="width:20%"><col style="width:23%"><col style="width:13%"><col style="width:13%"><col style="width:21%"><col style="width:10%"></colgroup><thead><tr><th>OA 编号 / 质量事件</th><th>反馈摘要 / 产品</th><th>提交人 / 时间</th><th>OA / 研判状态</th><th>当前处理 / 处理人</th><th>OA 附件</th></tr></thead><tbody>${current.map(r=>{const f=flows[r.id];return `<tr data-row="${r.id}" class="${state.selected===r.id?'selected':''}" aria-selected="${state.selected===r.id}"><td><button class="approval-link" data-select="${r.id}" aria-expanded="${state.selected===r.id}" aria-controls="detail">${icon('chevron')}${highlight(r.no)}</button><span class="event-id">${r.eventNo?highlight(r.eventNo):'尚未通报质量事件'}</span>${r.updated?'<span class="cell-sub">资料已更新 · V'+r.version+'</span>':''}</td><td><span class="summary-title">${highlight(r.title)}</span><div class="cell-sub">${esc(r.product)} · ${highlight(r.model)}</div></td><td>${esc(r.person)}<div class="cell-sub">${esc(r.date)}</div></td><td>${badge(r.approval,'approval')}<div style="margin-top:6px">${badge(r.review)}</div></td><td><span class="stage-button">${stageLabel(r)}</span><div class="stage-owner ${f?.tasks.some(t=>t.status==='returned')?'stage-alert':''}">${esc(stageOwner(r))}</div>${f?`<button class="text-btn event-row-link" data-enter-event="${r.id}">进入质量事件 ${icon('arrow')}</button>`:''}</td><td><button class="attachment-count ${r.fileIds.length?'':'none'}" data-attachments="${r.id}" aria-label="查看 ${esc(r.no)} 的原始附件">${icon('clip')}${r.fileIds.length} 份</button></td></tr>${state.selected===r.id?`<tr class="detail-row" data-detail-for="${r.id}"><td colspan="6"><div id="inlineDetailMount"></div></td></tr>`:''}`;}).join('')}</tbody></table></div>`;
...
const detailLabels={overview:'处理总览',attachments:'OA 附件',form:'原始表单',history:'审批与版本',review:'我的研判',analysis:'质量主管初析',tasks:'分配与承办',audit:'验收与动态'};
...
function detailFooter(r){
 if(viewState.surface==='feedback'&&!admissions[r.id]&&(r.oaStatus!=='RUNNING'||!r.activeForMa))return '<p>只有审批中且已到客服主管处理节点的记录才能进入质量流程。当前记录仅供查看。</p>';
 if(viewState.surface==='feedback')return `<p>${admissions[r.id]?'已选入质量处理；再次进入将打开当前研判或处理记录。':'确认选入后才加入“待我研判”，由 AI 辅助、客服主管作最终判断。'}</p><div class="footer-actions"><button class="btn primary" data-enter-event="${r.id}">进入质量事件 ${icon('arrow')}</button></div>`;
...
renderDetail=function(){
 ...
 const f=flows[r.id],raw=['attachments','form','history'].includes(state.tab),tabs=[['overview','处理总览'],['attachments','OA 原始资料'],['review','我的研判'],['analysis','质量主管初析'],['tasks','分配与承办'],['audit','验收与动态']];
 $('detail').innerHTML=`<header class="detail-heading"><div><div class="detail-eyebrow"><span>${f?'质量事件 '+r.eventNo:'质量处理 · '+r.review}</span>${tag('OA 关联事件')}<span>来源 V${r.version}</span></div><h2 class="detail-title">${esc(r.title)}</h2><div class="detail-meta"><span>OA 审批 ${esc(r.no)}</span><span>${esc(r.person)} · ${esc(r.date)}</span><span>${badge(r.approval,'approval')} ${badge(r.review)}</span></div></div><div class="detail-tools"><button class="text-btn" data-action="copy-number">${icon('copy')}复制 OA 编号</button><button class="text-btn muted" data-action="back">${icon('up')}收起详情</button></div></header><div class="detail-tabs"><div class="tabs" role="tablist" aria-label="反馈及后续处理">${tabs.map(([id,label])=>`<button class="tab" role="tab" aria-selected="${id===state.tab||id==='attachments'&&raw}" data-tab="${id}">${label}${id==='attachments'?'<span class="count">'+r.fileIds.length+'</span>':''}</button>`).join('')}</div></div><div class="detail-flow" id="detailFlow">${phasePath(r)}</div><div class="detail-content" id="detailContent" role="region"></div><footer class="detail-footer"><p>${f?'后续处理由各环节负责人完成；客服主管在此查看结果与进度。':'员工提交即接入；正式通报后，继续在同一条记录中跟踪处理。'}</p><div class="footer-actions"><button class="btn" data-action="oa">${icon('out')}查看 OA 原单</button>${state.tab!=='overview'?'<button class="btn" data-tab="overview">返回处理总览</button>':!f&&r.review!=='普通反馈'?'<button class="btn primary" data-tab="review">'+(r.review==='待补资料'?'查看补充要求':'进入人工研判')+icon('arrow')+'</button>':''}</div></footer>`;
...
function renderFeedbackDetail(r){
 ...
 $('detail').innerHTML=`<header class="detail-heading"><div><div class="detail-eyebrow"><span>OA 原始事件</span>${tag('来源只读')}<span>V${r.version}</span></div><h2 class="detail-title">${esc(r.title)}</h2><div class="detail-meta"><span>OA 审批 ${esc(r.no)}</span><span>${esc(r.person)} · ${esc(r.date)}</span>${badge(r.approval,'approval')} ${tag(admissions[r.id]?'已选入质量处理':'尚未选入研判')}</div></div><div class="detail-tools"><button class="text-btn" data-action="copy-number">${icon('copy')}复制编号</button><button class="text-btn muted" data-action="back">${icon('up')}收起详情</button></div></header><div class="detail-tabs"><div class="tabs" role="tablist" aria-label="原始事件资料">${[['form','详细事件'],['attachments','OA 附件 '+r.fileIds.length],['history','审批与版本']].map(([id,label])=>`<button class="tab" role="tab" data-tab="${id}">${label}</button>`).join('')}</div><span class="quiet">先核对资料，再确认选入</span></div><div class="detail-content" id="detailContent" role="region"></div><footer class="detail-footer">${detailFooter(r)}</footer>`;
 syncDetailNavigation();renderDetailContent(r);
}
...
function setSurface(surface){
 ...
 document.title=event?'质量事件 '+(currentRow()?.eventNo||'')+' · 客服主管视角':'全部事件 · 客服主管工作台';
}
...
function phasePath(r){
 ...
 const phases=[['提交接入','attachments','员工 / OA'],['AI 与人工研判','review','客服主管'],['质量初析','analysis','质量主管'],['任务分配','tasks','主管'],['员工承办','tasks','承办员工'],['质量终验','audit','质量主管']];
...
function renderTasks(r){if(flows[r.id]?.example)return renderExampleTasks(r);
 ...
 if(!f.tasks.length)return `${note('分配和承办结果来自原任务系统。在客服主管视角可以查看，不在这里代替主管分配。')}<div class="manager-line"><span class="avatar">${f.mode==='analysis'?'佟':'管'}</span><div><h3>${f.mode==='analysis'?'等待质量主管完成初析':'已完成初析，等待主管确认分配'}</h3><p>${f.mode==='analysis'?'初析提交后再进入任务分配。':f.department+' · 建议期限 '+f.due}</p></div></div>${emptyStage('尚未发放员工任务',f.mode==='analysis'?'本环节暂未开始，不显示预设承办人或虚构分配结果。':'主管确认承办人和任务要求后，具体事项、承接结果与执行进度会显示在这里。','<button class="btn" data-tab="analysis">查看质量主管初析</button>')}`;
...
function renderAudit(r){if(flows[r.id]?.example)return renderExampleAudit(r);
 ...
 if(sub==='results')return html+(f.mode==='closed'?`<div class="quality-outcome"><h3>${icon('check')} 质量终验通过 · 事件已关闭</h3><p>${esc(f.final)}</p><p class="quiet">终验人：质量主管 · ${f.closedAt} · 历史版本继续保留</p></div><div class="facts-grid" style="margin-bottom:24px">${textFact('最终分类',f.analyses.at(-1).category)}${textFact('最终原因（演示）','记录校验环节存在遗漏，已修正并补齐复核。')}${textFact('措施（演示）','更新检查要求并补齐记录。')}${textFact('验证结果（演示）','对照与独立复核已通过。')}</div>`:`<div class="status-banner"><div><span class="eyebrow">质量终验 · 质量主管</span><h3>${f.mode==='quality'?'等待质量终验':'尚未进入终验'}</h3><p>${f.mode==='quality'?'主管验收已通过，等待质量主管核对证据后作最终判断。':'需要先完成任务执行和主管验收，再进入质量终验。'}</p></div>${tag(f.mode==='quality'?'待终验':'前序环节处理中','blue')}</div>`)+`<div class="section-head"><h3>逐项主管验收结果</h3><span class="quiet">主管验收和质量终验分别记录</span></div>${f.tasks.length?f.tasks.map(t=>`<div class="record-line"><div>${tag(taskStatus[t.status][0],taskStatus[t.status][1])}</div><div><h3>${esc(t.title)}</h3><p>${esc(t.supervisor)}</p><button class="text-btn" data-task="${t.id}" style="font-size:12px;min-height:30px">查看任务与证据</button></div><time class="quiet">${t.reviewed?f.manager+' · '+t.reviewed:'尚无主管验收记录'}</time></div>`).join(''):emptyStage('尚无主管验收记录','任务尚未发放。')}`;
...
showGuide=function(){info('客服主管视角 · 交互评审',`<p>查看所有已接入反馈，并持续跟踪关联事件。下列人员、任务、结论和文件均为演示内容。</p><div class="scenario-list">${[['a5','员工处理中','查看质量主管 V2 初析、3 项任务与退回补充'],['a4','待质量主管初析','已通报，尚未提交初析结果'],['a11','待主管分配','初析已完成，尚未发放员工任务'],['a7','待质量终验','任务均通过主管验收'],['a12','已关闭','查看最终结论、证据与历史记录'],['a1','待我研判','查看 4 份 OA 附件，体验保存草稿和通报']].map(([id,title,desc])=>`<button class="btn" data-scenario="${id}"><span>${title}<small>${desc}</small></span>${icon('arrow')}</button>`).join('')}</div><button class="text-btn" data-reset-demo>重置全部演示状态</button><p>所有演示变更仅在本页内生效。客服主管在后续流程中只查看，不代替质量主管、主管或员工提交业务操作。</p>`);$('closeInfo').textContent='关闭';};
...
document.title='客服主管工作台 · 反馈与全流程跟踪';
document.querySelector('.breadcrumb').innerHTML='质量追踪 <span style="margin:0 8px">/</span> 客服主管工作台';
