...
const exampleScenes={
 assigned:{label:'已分配 / 待承接',mode:'active',description:'主管已向本部门员工发放 4 项任务，员工尚未确认承接。'},
 execution:{label:'执行与退回',mode:'active',description:'1 项通过、1 项执行中、1 项退回补充、1 项待承接。'},
 quality:{label:'待质量终验',mode:'quality',description:'4 项任务均通过主管验收，证据包已送质量主管终验。'},
 returned:{label:'终验退回',mode:'active',description:'质量主管将回归验证节点退回补充，历史证据继续保留。'},
 closed:{label:'完整结果 / 已关闭',mode:'closed',description:'查看初析、分配、承办、证据和终验的完整结果。'}
};
const exampleDepartment='软件研发部（模拟）';
const exampleManager='程远（模拟主管）';
const exampleOwners=['陈昊（模拟）','林悦（模拟）','周岚（模拟）','许晴（模拟）'];
const exampleTaskSpecs=[
 {title:'定位大容量图像导出的性能瓶颈',department:exampleDepartment,manager:exampleManager,requirement:'复现 128 MB、512 MB、1 GB 三档数据导出，拆分读取、编码与写盘耗时，提交性能采样和定位报告。',acceptance:'同一设备、同一数据集重复 5 次；记录各阶段耗时及环境，定位依据可复核。',dependency:'无前置任务。',file:'导出链路性能定位报告.txt',result:'确认耗时主要集中在逐帧同步写入与高频进度刷新，已提交 5 轮基线记录。'},
 {title:'优化分批写入与导出进度反馈',department:exampleDepartment,manager:exampleManager,requirement:'根据定位结论完成批量写入及进度刷新优化，处理导出取消场景，交付修订记录和开发自测证据。',acceptance:'1 GB 演示数据导出不超过 35 秒；取消操作可响应，已写入文件处理符合约定。',dependency:'以前序性能定位结果作为输入。',file:'批量写入优化与自测记录.txt',result:'已完成批量写入和进度刷新优化，1 GB 数据自测 31.7 秒；取消操作自测通过。'},
 {title:'完成容量、介质及取消场景回归',department:exampleDepartment,manager:exampleManager,requirement:'覆盖 3 档数据容量、本地磁盘与外接介质，核对导出文件完整性，并补充取消场景与对照复测记录。',acceptance:'20 项功能检查全部通过；文件数量与校验值一致；慢速介质的进度提示清晰可读。',dependency:'使用优化版本执行回归；保留原版本作为对照。',file:'导出功能回归验证记录.txt',result:'完成 20 项功能回归，文件数量与校验值一致，补齐慢速介质和取消场景对照。'},
 {title:'完成使用场景复核与结果归档',department:exampleDepartment,manager:exampleManager,requirement:'由本部门员工按回归通过的版本复核完整导出场景，记录环境、版本与操作结果，并归档验收材料。',acceptance:'场景复核记录完整，版本与设备对应清楚，导出结果可正常打开，材料可供主管整体验收。',dependency:'需等待软件优化与回归通过后进行使用场景复核。',file:'导出使用场景复核与归档记录.txt',result:'已完成部门内使用场景复核，导出图像可正常打开，版本与环境记录齐全，归档材料已提交主管。'}
];
...
function buildFlowExample(r,scene='closed'){
 ...
 const f={example:true,scene,mode:selected.mode,manager:exampleManager,department:exampleDepartment,due:'2026-09-10 18:00',handoffAt:'2026-09-06 14:40',assignedAt:'2026-09-07 09:00',reviewTime:'2026-09-05 14:00',updated:scene==='assigned'?'2026-09-07 09:00':scene==='execution'?'2026-09-08 10:40':scene==='returned'?'2026-09-08 14:30':'2026-09-08 14:50',analyses:[],tasks:[],activity:[]};
 const analysis={version:1,author:'质量主管',time:'2026-09-06 09:20',category:'软件与数据功能 / 数据、报告与测量',direction:'围绕大容量图像导出的处理耗时、文件完整性和操作反馈开展分析。',facts:'员工反馈“图像导出时间较长”，来源关联服务日志；后续复测数据均为本原型独立模拟。',basis:'演示复测中，数据容量增加时耗时明显增长；应拆分读取、编码及写盘耗时，区分程序处理与外部存储的影响。',conclusion:'初步指向导出链路的同步写入和刷新频率，需要通过性能采样、优化及对照回归验证。',gap:'需补充三档容量的性能采样、慢速介质与取消操作记录。',requirements:'1. 定位主要耗时环节，保留复现环境和基线。\n2. 形成优化措施并完成开发自测。\n3. 对照验证导出耗时和文件完整性。\n4. 收集使用场景复核结果。',department:exampleDepartment,due:f.due};
 f.analyses=[analysis,{...analysis,version:2,time:'2026-09-06 14:30',basis:'已核对演示性能采样：1 GB 数据原版本导出约 95.8 秒，需重点核查逐帧写入与高频界面刷新。',conclusion:'将性能定位、批量写入优化、回归验证和使用场景复核分别形成可验收事项；最终根因以对照结果确认。',requirements:analysis.requirements+'\n新增：覆盖 128 MB、512 MB、1 GB 及本地 / 外接介质；记录取消场景。',reason:'补充容量、介质和取消场景，防止仅验证常规路径。',sync:'质量主管确认唯一主责部门为软件研发部，09-06 14:40 推送该部门主管程远；主管于 09-07 09:00 在原任务系统确认分配。'}];
 ...
 f.tasks=exampleTaskSpecs.map((spec,index)=>{
  ...
  if(scene==='returned'&&index===2){
   ...
   t.history.push({at:'2026-09-08 12:20',title:'补交证据 V2',by:t.person,body:'补齐常规回归对照后重新提交。'},{at:'2026-09-08 13:00',title:'主管验收通过',by:spec.manager,body:'主管核对 V2 后通过验收。'},{at:'2026-09-08 14:30',title:'质量终验指定节点退回',by:'质量主管',body:'需增加低速介质的原始采样记录，其他节点结果保留。'});
   t.history.sort((a,b)=>a.at.localeCompare(b.at));t.qualityReturned=true;
  }
  return t;
 });
 f.activity=[{category:'研判',time:f.reviewTime,person:'客服主管',title:'完成分类与风险研判并推送初析',text:'分类：软件与数据功能 / 数据、报告与测量；风险：中。',tab:'review'},...f.analyses.map(a=>({category:'初析',time:a.time,person:'质量主管',title:'提交质量初析 V'+a.version,text:a.version===1?'形成分析方向、信息缺口及建议责任部门。':a.reason,tab:'analysis'})),{category:'分配',time:f.handoffAt,person:'质量主管',title:'确认唯一主责部门并推送对应主管',text:'初析 V2、必须成果清单及建议期限已推送软件研发部主管程远的原任务规划入口。',tab:'tasks'},{category:'分配',time:f.assignedAt,person:f.manager,title:'部门主管在原任务系统发布 4 项任务',text:'唯一主责部门：软件研发部；程远主管确认 4 位部门员工的承办事项、交付物与期限。',tab:'tasks'}];
 ...
 if(['quality','returned','closed'].includes(scene))f.activity.push({category:'验收',time:'2026-09-08 13:20',person:f.manager,title:'部门主管整体验收并提交证据包',text:'4 项任务通过本部门主管逐项验收，主管核对整体验收要求后提交质量主管质量终验。',tab:'audit'});
 ...
 if(scene==='closed'){f.closedAt='2026-09-08 14:50';f.final='质量主管核对定位依据、优化措施、主管验收及验证记录后通过质量终验。本事件关闭，初析与证据历史版本继续保留。';f.activity.push({category:'终验',time:f.closedAt,person:'质量主管',title:'质量终验通过，关闭事件',text:f.final,tab:'audit'});}
 return f;
}
function seedFlowExamples(){
 const r=rows.find(x=>x.id==='a8');if(!r)return;
 r.review='已通报';r.eventNo='QT-DEMO-20260905-008';r.manualConclusion='根据图像导出耗时问题，确认分类为软件与数据功能 / 数据、报告与测量，风险为中；推送质量主管进一步分析。';
...
function seedExampleAssessment(){
 ...
 a.finalized=true;a.dirty=false;a.adoption='MANUAL';a.saved=[{version:1,sourceVersion:r.version,at:'2026-09-05 14:00',by:'客服主管（演示）',draft:structuredClone(a.draft),adoption:'MANUAL',aiId:null}];
}
...
function exampleTaskNavigation(){const assigning=viewState.viewedPhase!==4;return `<div class="subnav example-task-nav"><button class="chip ${assigning?'active':''}" data-example-task-view="3">主管分配结果</button><button class="chip ${!assigning?'active':''}" data-example-task-view="4">员工承办结果</button><span class="quiet">模拟正式任务结果 · 客服主管只读查看</span></div>`;}
function renderSingleDepartmentHandoff(f){return `<div class="assignment-owners single-department"><article><span class="owner-marker">责</span><div class="single-department-content"><div class="department-facts">${textFact('唯一主责部门',f.department)}${textFact('对应部门主管',f.manager)}${textFact('初析确认推送','质量主管 · '+f.handoffAt)}</div><p>初析 V2、必须成果清单与建议期限已推送该部门主管；具体任务由主管在原任务系统确认发布。</p></div></article></div>`;}
function renderExampleTasks(r){
 const f=flows[r.id],assigning=viewState.viewedPhase!==4;
 if(assigning)return exampleTaskNavigation()+`<div class="example-heading"><div><span class="eyebrow">主管确认结果</span><h3>${f.department} · 4 项部门内任务</h3><p>${f.manager}在原任务系统规划并发布 · 依据质量主管初析 V2 · ${f.assignedAt}</p></div>${tag('已确认分配','green')}</div>${renderSingleDepartmentHandoff(f)}<div class="section-head"><h3>具体分配事项</h3><span class="quiet">人员、期限与验收标准均来自模拟分配结果</span></div><div class="assignment-table-wrap"><table class="assignment-table"><thead><tr><th>任务 / 交付物</th><th>承办人 / 分配主管</th><th>截止时间</th><th>当前状态</th><th>查看</th></tr></thead><tbody>${f.tasks.map(t=>`<tr><td><small>${t.number}</small><b>${esc(t.title)}</b><span>${esc(t.file)}</span></td><td><b>${t.person}</b><small>${t.manager}</small></td><td>${t.due.slice(5)}<small>${t.department}</small></td><td>${tag(taskStatus[t.status][0],taskStatus[t.status][1])}</td><td><button class="text-btn" data-task="${t.id}">承办结果 ${icon('arrow')}</button></td></tr>`).join('')}</tbody></table></div><details class="assignment-memo" open><summary>分配依据与任务衔接</summary><p>质量主管初析 V2 确认唯一主责部门；对应主管在原任务系统规划任务，再把具体事项分配给本部门员工。</p><div class="handoff-line"><span>性能定位</span>${icon('arrow')}<span>导出优化</span>${icon('arrow')}<span>回归验证</span>${icon('arrow')}<span>使用场景复核</span></div><p class="quiet">箭头表示交付物输入关系。承接状态、执行进度和验收结果分别记录。</p></details>`;
...
function renderExampleAudit(r){
 ...
 const started=q.state!=='not_started',title=q.state==='passed'?'质量终验通过 · 事件已关闭':q.state==='returned'?'质量终验退回 · 指定节点补充':q.state==='pending'?'主管验收已完成 · 等待质量主管终验':'员工任务处理中 · 尚未进入质量终验';
 return nav+`<section class="quality-review-banner ${q.state}"><div><span class="eyebrow">质量终验 · 质量主管</span><h3>${title}</h3><p>${q.state==='passed'?f.final:q.state==='returned'?q.returnReason:q.state==='pending'?'任务结果与证据包已齐备，待质量主管逐项核对后填写终验结论。':'当前显示已有主管反馈；完成任务并通过主管验收后，再送质量终验。'}</p></div>${tag(q.state==='passed'?'已通过':q.state==='returned'?'退回补充':q.state==='pending'?'待终验':'未开始',q.state==='passed'?'green':q.state==='returned'?'amber':'blue')}</section>${q.state==='returned'?`<div class="quality-return-node"><div><b>退回节点：完成容量、介质及取消场景回归</b><p>承办：周岚（模拟） · 主管：${f.manager}</p><small>2026-09-08 14:30 · 其他 3 项任务结果保留，V1 / V2 证据均可追溯。</small></div><button class="btn" data-task="${r.id}-t3">查看退回节点</button></div>`:''}${started?`<div class="quality-verdict-grid"><section><div class="section-head"><h3>终验核对项</h3><span class="quiet">${q.state==='pending'?'待质量主管确认':'已记录核对意见'}</span></div>${q.checks.map(([label,text],i)=>`<div class="quality-check"><span class="check-dot ${q.state==='pending'?'waiting':q.state==='returned'&&i===2?'issue':''}">${q.state==='pending'?'○':q.state==='returned'&&i===2?'!':'✓'}</span><div><b>${label}</b><p>${text}</p></div>${tag(q.state==='pending'?'待核对':q.state==='returned'&&i===2?'需补原始记录':'已核对',q.state==='pending'?'blue':q.state==='returned'&&i===2?'amber':'green')}</div>`).join('')}</section><section class="quality-findings"><h3>${q.state==='passed'?'最终归档结论':'待终验核实的处理摘要'}</h3><div>${textFact('问题分类',f.analyses.at(-1).category)}${textFact('原因',q.rootCause)}${textFact('措施',q.measures)}</div><p class="quiet">${q.state==='passed'?'质量主管 · '+f.closedAt:'上述内容来自承办结果汇总，质量结论以终验确认为准。'}</p></section></div><section class="validation-results"><div class="section-head"><h3>对照验证结果 · 模拟数据</h3><span class="quiet">同一测试环境 · 数值仅用于界面评审</span></div><table><thead><tr><th>数据容量</th><th>优化前</th><th>优化后</th><th>核对结果</th></tr></thead><tbody>${[['128 MB','12.6 秒','5.1 秒'],['512 MB','48.2 秒','16.4 秒'],['1 GB','95.8 秒','31.7 秒']].map(a=>`<tr><td>${a[0]}</td><td>${a[1]}</td><td><b>${a[2]}</b></td><td>${q.state==='returned'?'需补低速介质原始记录':'文件数量与校验值一致'}</td></tr>`).join('')}</tbody></table><p class="quiet">${q.validation} ${q.state==='passed'?'质量主管已核对本组记录。':'以上为员工提交的结果，质量终验尚未通过。'}</p></section>`:''}<section class="supervisor-results"><div class="section-head"><h3>逐项主管验收结果</h3><span class="quiet">主管验收和质量终验分别记录</span></div>${f.tasks.map(t=>`<div class="record-line"><div>${tag(t.qualityReturned?'主管曾验收通过':taskStatus[t.status][0],t.qualityReturned?'green':taskStatus[t.status][1])}</div><div><h3>${t.title}</h3><p>${t.qualityReturned?'V2 已通过主管验收，现按质量终验意见补充原始采样。':t.supervisor}</p><button class="text-btn" data-task="${t.id}">查看承办与证据</button></div><time class="quiet">${t.manager}<br>${t.reviewed||'尚未验收'}</time></div>`).join('')}</section>`;
}
...
function initializeFlowExamples(){
 ...
 showGuide=function(){originalGuide();$('infoBody').insertAdjacentHTML('afterbegin','<div class="example-guide-link"><b>新增：完整后续流程样例</b><p>图像导出时间较长 · 初析 V1/V2、单部门主管分配、4 项员工任务、证据与质量终验。</p><button class="btn primary" data-open-flow-example>打开完整流程演示 '+icon('arrow')+'</button></div>');};
