...
- **短期记忆**：`knownFacts[]`（session-store），模型通过 `update_known_facts` / `list_known_facts` 自主维护；有未发布草案时每轮 `[memory_context]` 注入完整 **`latestDraft`**（`ORCHESTRATOR_DRAFT_MEMORY_MAX_CHARS` 可截断）；**拆细/扩条**须整表 `draft` JSON，**单点改**用 `update_draft_task` / `remove_draft_subtask`（v5.23.5 prompt-first，无新增用户话术正则）。
