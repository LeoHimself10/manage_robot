...
- **提示词**：钉钉 / ReAct 主链路使用 `orchestrator-agent-v5.11`（`buildQwenPlannerSystemPrompt`，含 planner / manager / employee profile）；`generateStructuredPlan` 单次 JSON 链路（`demo` CLI、`demo:eval`、调试脚本）使用独立的 `legacy-demo-planner-v1`（`buildLegacyDemoPlannerSystemPrompt`），描述完整 classification / tasks[*].id / capaAdvisory(QUALITY) / gateSelfCheck schema，与 orchestrator 解耦。
