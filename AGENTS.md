...
- **提示词**：钉钉 / ReAct 主链路使用 `orchestrator-agent-v5.20.1`（`buildQwenPlannerSystemPrompt`，含 planner / manager / employee profile）；身份定位为约 350 人医疗器械（OCT）公司内部任务规划助手，以质量与研发为主，但可按用户本轮描述扩展到其他业务部门。主管/admin 链路在 `managerFollowup` 注入时增加第六模式 **FOLLOWUP**（催办/逾期跟进）；默认 planner 为 **CLARIFY / QUERY / DRAFT / ASSIGN / PUBLISH** 五类操作模式：CLARIFY 禁止与其他模式混用；QUERY 用于正式任务查询（`list_managed_tasks` 等）；DRAFT / ASSIGN / PUBLISH 可按同句叠加；**禁止在 message 中输出任务表**，钉钉侧任务表由服务端根据 JSON `draft` 自动渲染（`renderDingtalkTaskMarkdown`）。纪律要点：**禁止未调 `publish_task` 即声称已发布**、**未调 `start_new_task` 即声称已归档/已切换**；**DRAFT 前必须已有明确截止日期或可执行时间范围**；**DRAFT 必须 JSON 顶层 `draft` 落盘**（避免 `hasDraft=false`）；**任何姓名须先调 `search_employees`**。`generateStructuredPlan` 单次 JSON 链路使用独立的 `legacy-demo-planner-v1`，与 orchestrator 解耦。
...
- **Prompt**：第六模式 `FOLLOWUP`（`orchestrator-agent-v5.20.1`，`managerFollowup` 注入）；与 `QUERY` 只读查询区分。
