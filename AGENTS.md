...
- 当前 Demo/MVP 主链路：**基础输入护栏**（空输入等 + **`INPUT_MAX_CHARS` 超长则追问、不静默截断**）→ **Qwen 结构化输出**（**`responseIntent`/`assistantMessage`**、追问与场景分类、任务包/WBS、质量域 CAPA 建议、门禁自检；可选 **`sessionDigest` 与 `conversationState` 上轮摘要**）→ **结构校验与派发门禁二次确认**（含 **`consistency` 依赖/环等 warnings 合入 `gate.warnings`**）→ **Markdown/表格输出**（`DRAFT_READY` 前对正文做 **手机号/身份证/IPv4** 级脱敏，**无**政治词黑名单）→ **JSONL 审计落盘**（每轮 `createTaskPlanningDemo` 完结一行，见 `AUDIT_DEMO_*`）与可选 **Plan 文件快照**（`PLAN_STORE_DIR`）。须配置 `QWEN_API_KEY`（见 `.env.example`）。**不存在**关键词分类、规则 CAPA、subtype 固定骨架等无模型生成路径；校验或模型失败返回 `GENERATION_FAILED`，**不用规则稿顶替**。
- **钉钉 Stream 试点**（`npm run dingtalk-bot`，部署见 **`docs/deploy-aliyun-dingtalk.md`**）：单次用户消息 **仅一条 Markdown 终稿气泡**（闲聊 / 追问 / 讨论 / 拆解稿 / 失败说明），**不向用户推送**「处理中」或流式进度等中间气泡。自然回复来自模型 **`assistantMessage`**，结构化追问可来自 **`openQuestions`**；钉钉侧**不**自动加列表符号或固定追问标题。`QWEN_STREAM` **默认 SSE**（`QWEN_STREAM=0` 关；仅服务端拼装 JSON，与钉钉展示条数无关）。提示词版本见 `src/agent/demo/qwen-prompt.ts`（当前 **`task-planning-agent-v2.11.0`**）；模型 JSON 含 **`responseIntent`**（`CHAT`|`CLARIFY`|`DISCUSS`|`DRAFT`|`REVISE_DRAFT`|`RESET_OR_NEW_TASK`）与 **`assistantMessage`**；可选 **`clarificationUx`**（`NON_TASK`|`TASK_GAP`）。Windows 与服务端一键更新：`scripts/ecs-deploy-dingtalk.ps1`。
