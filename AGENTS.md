...
- **短期记忆**：`knownFacts[]`（session-store），模型通过 `update_known_facts` / `list_known_facts` 自主维护；有未发布草案时每轮 `[memory_context]` 注入完整 **`latestDraft`**（`ORCHESTRATOR_DRAFT_MEMORY_MAX_CHARS` 可截断），局部修订须 `update_draft_task` / `add_draft_subtask` / `remove_draft_subtask`，避免无工具整表重出 `draft.tasks[]`。
