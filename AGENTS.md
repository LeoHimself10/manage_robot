...
- **提示词**：钉钉 / ReAct 主链路使用 `orchestrator-agent-v6.3.6`（`buildQwenPlannerSystemPrompt`，含 planner / manager / employee profile）；阶段 B/C-1/C-2 **必须**同轮输出顶层 `draft`（`tasks.length >= 1`），`message` 仅短摘要；纪律要点：**禁止未调 `publish_task` 即声称已发布**、**未调 `start_new_task` 即声称已归档/已切换**（仅用户明确「新任务/换个任务」时预清旧 `candidatePool`/花名册/草案，避免旧人员文档污染）；**首轮必问期望完成时间/截止日期**（已答则不重复）；**确认/确认发布/发布吧**等宽泛肯定词触发发布、**再改/等等/取消**等否定词禁止发布；**message 与 JSON 顶层 `draft` 同轮落盘**（避免 `hasDraft=false` 断链；线上 `DRAFT_FALLBACK_EXTRACT_ENABLED=1` 时，message-only 长草案会二次抽取补 `draft`）；**上传花名册后优先 `match_roster_to_contacts` 批量匹配写候选池，禁止对名单每人 `search_employees`**；**候选池内以 `search_employees` 为准、禁止「未找到」与身份信息自相矛盾**（纯 prompt，无代码正则兜底）。`generateStructuredPlan` 单次 JSON 链路（`demo` CLI、`demo:eval`、调试脚本）使用独立的 `legacy-demo-planner-v1`（`buildLegacyDemoPlannerSystemPrompt`），描述完整 classification / tasks[*].id / capaAdvisory(QUALITY) / gateSelfCheck schema，与 orchestrator 解耦。
- **工具**（`src/agent/tools/`）：按 profile 暴露。ReAct 主链路已改为**最终 JSON 直接输出 `draft`**（不再依赖 `save_draft` 工具回合）；为了兜底「模型只产 Markdown 表格不产结构化 draft」的退化情况，`prepare_publish_task` 在校验通过时**会把规整后的 draft + assignment 直接写入 `currentSession.latestDraft` / `latestAssignment`**，下一轮的 `publish_task` 才能读到结构化数据。`planner` 含 `search_employees` / `get_employee_details` / `read_uploaded_roster_text` / **`match_roster_to_contacts`**（花名册批量 `searchContacts`，**不占** `SEARCH_EMPLOYEES_PER_ORCHESTRATOR_QUOTA）/ `search_similar_plans`（`SEARCH_SIMILAR_PLANS_ENABLED=0` 时关闭）/ 条件开放 `search_web` / `get_current_time` / `update_known_facts` / `list_known_facts`；`manager` 在此基础上提供发布与管理工具；`admin` 再追加管理视角工具；`employee` 提供 `list_my_tasks` / `submit_employee_response` / `submit_progress_update` / `update_employee_profile` 等员工侧工具。`search_web` 受 `SEARCH_WEB_ENABLED` 与请求语义双重约束。`search_employees` 默认返回 cap 收紧到 **25**（`SEARCH_EMPLOYEES_MAX_CANDIDATES`），并对**单次 orchestrator 内调用次数**设硬上限 **3**（`SEARCH_EMPLOYEES_PER_ORCHESTRATOR_QUOTA`），超过即返回 `ok:false / search_employees_quota_exhausted` 让模型直接转述，防止"反复换参数搜不到 → ReAct 死循环 / token budget 爆栈"。`publish_task` 遇到 empty draft / missing assignee 不再 throw，改为 `ok:false / no_draft_in_session | missing_assignee` 软返回；成功发布时 `notifyPublishedTask` 传入 DB 已写入的 **`taskDescription`**（`tasks.description`）与子任务 `extra`（含 v2 字段）。
- **钉钉会话与多任务**：同一钉钉会话（`chatKey`）内 `PlanSession` 使用 `taskScopes` 与顶层字段跟踪当前规划；**`planId`** 标识当前规划轮次（与激活 scope 对齐）。`start_new_task` 会**清空**当前未发布草案/分配/短期记忆并生成新 `planId`；**未发布的旧草案不可通过聊天恢复**。`**publish_task` 成功（非 `alreadyPublished` / 非 LRU 去重 / 非 `unknown_assignees`）** 后会自动轮转 **`planId`**，并在内部镜像已发布 scope 供审计；同一会话可连续规划下一条正式任务。已发布任务请用 `list_managed_tasks` / `get_task_detail` 查询。`**DINGTALK_PLANID_ROTATE_ENABLED=0**` 可关闭发布后自动轮转（默认 `1`）。
...
- **上线前 Agent Eval（硬性门禁）**：部署钉钉生产镜像前须 `QWEN_API_KEY` 齐备且 **`npm run eval:agent`**（`scripts/run-agent-eval.ts`）全绿。脚本对齐钉钉主链路（`runOrchestrator`、`resolveDingtalkAgentRouting`、`DINGTALK_QWEN_*`），隔离 `EVAL_DATA_DIR`，默认 `WORKBENCH_DINGTALK_NOTIFY_ENABLED=0`。场景含 P1–P5 规划、**P2b v6 草案**、**R1 花名册 `match_roster_to_contacts`**（`search_employees` ≤1、禁止配额道歉）、**R2 按池分配**、**P6 `start_new_task`**、**M1/M2 发布专检**（M1 禁止 `publish_task` 与假发布文案；M2 要求 `publish_task` + SQLite `tasks.description` / `subtasks.extra_json` v2）、M3–M5 与 E1–E4 运维。建议与 ECS 一致：`DINGTALK_ORCHESTRATOR_MAX_ITERATIONS=10`、`SEARCH_EMPLOYEES_PER_ORCHESTRATOR_QUOTA=3`、`DINGTALK_ROLE_ROUTING_ENABLED=1`。摘要写入 `EVAL_DATA_DIR/eval-summary.json`；`eval FAILED` 时禁止推生产。
