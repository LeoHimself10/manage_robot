...
- **运行时数据源约束**：工作台正式任务仅以 SQLite 为权威源；`tasks.json` 不参与运行时查询与回灌迁移。`subtasks` 表可选 **`extra_json`**（TEXT），发布时从草案写入 `{ v:1, dependsOn?, checkpoints?, risks? }`（分别对应 `dependencyTaskIds` / `timeNode.checkpoints` / `risksAndOpenQuestions`），供详情接口与钉钉通知展示。员工画像与通讯录快照也已落在 SQLite 数据层（`people-directory-store`、`dingtalk_contacts`）。**工作台子任务状态**：员工「接受」后子任务直接为 **`IN_PROGRESS`**（不再落库 **`ACCEPTED`**）；启动时会把历史 **`ACCEPTED`** 行迁移为 **`IN_PROGRESS`**，审计事件名 **`SUBTASK_ACCEPTED`** 仍保留。
