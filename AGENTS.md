...
- 当前 Demo/MVP 主链路：**输入质检** → **Qwen 结构化输出**（场景分类、任务包/WBS、质量域 CAPA 建议）→ **字段校验与派发门禁** → **Markdown/表格输出**；须配置 `QWEN_API_KEY`（见 `.env.example`）。**不存在**关键词分类、规则 CAPA、subtype 固定骨架等无模型生成路径；校验或模型失败返回 `GENERATION_FAILED`，**不用规则稿顶替**。
- **确定性规则**仅用于约束 AI：输入质检、`llm-schema`（如质量域必含 CAPA、研发域不含）、`gate`；不负责在无 LLM 时生成完整草案。
