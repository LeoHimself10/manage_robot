...
type EventSeed = {
  id: string;
  eventNo: string;
  status: "PENDING_ANALYSIS" | "PENDING_ASSIGNMENT" | "PENDING_ACCEPTANCE" | "IN_PROGRESS" | "PENDING_PRIMARY_REVIEW" | "PENDING_QUALITY_REVIEW" | "CLOSED";
  title: string;
  purpose: string;
  analysis: boolean;
  nodes: NodeSeed[];
...
try {
  for (const item of events) {
    const eventOrdinal = Number(item.eventNo.slice(-3));
    const sourceKey = `quality-test-source:${item.eventNo}`;
    const sourceSnapshot = {
      feedbackNo: `TEST-${item.eventNo}`,
      feedbackAt: occurredAt,
      reporter: "测试反馈人",
      deviceModel: "测试设备型号",
      serialNo: "TEST-SN-001",
      catheterBatch: "TEST-BATCH-001",
      issueDescription: item.purpose,
      clinicianAware: "已知悉",
      impact: "仅影响隔离测试数据",
      confirmation: "隔离测试事实已确认",
    };
    const contentHash = createHash("sha256").update(JSON.stringify(sourceSnapshot)).digest("hex");
    db.prepare(`
    ...
    const aiAssessmentId = `quality-test-ai:${item.eventNo}`;
    const aiOutput = {
      schemaVersion: "ai-original-assessment-output-v0",
      requestId: `quality-test-ai-request-${item.eventNo}`,
      handlingRecommendation: "QUALITY_ANOMALY",
      primaryCategoryCode: "IMAGING_OPTICS",
      secondaryCategoryCode: "IMAGE_NONE_INTERRUPTED",
      riskLevel: "MEDIUM",
      reasoningBasis: [
        { statement: "反馈描述涉及影像或质量记录异常，需要进入隔离测试后续流程核验。", citationIds: ["feedback-1"] },
        { statement: "当前测试数据已提供型号、问题描述和影响范围。", citationIds: ["feedback-1"] },
      ],
      similarCases: [],
      missingInformation: eventOrdinal % 3 === 0
        ? [{ field: "复现记录", reason: "需要在测试执行阶段补充证据" }]
        : [],
      uncertainties: [{ topic: "最终根因", reason: "需由责任部门完成验证后确认" }],
      citations: [{
        citationId: "feedback-1",
        sourceType: "FEEDBACK",
        sourceId: sourceKey,
        description: "隔离测试来源事实",
      }],
      provenance: {
        modelConfigId: "quality-test-fixture",
        promptVersion: "quality-test-fixture",
        categoryDictionaryVersion: "historical-feedback-taxonomy-v0",
        caseLibraryVersion: "quality-test-fixture",
      },
    };
    db.prepare(`
      ...
      ) VALUES(?,?,1,?,?,?,'[]','QUALITY_TEST_AFTERSALES_001',?)
      ON CONFLICT(id) DO NOTHING
    `).run(
      aiAssessmentId,
      sourceKey,
      `quality-test-ai-request-${item.eventNo}`,
      JSON.stringify(sourceSnapshot),
    ...
    db.prepare(`
      INSERT INTO quality_source_assessments(
        ...
        'STANDARD',NULL,NULL,'MEDIUM',?,'MODIFIED',?,?,
    ...
    db.prepare(`
      ...
      ) VALUES(?,'REPORTED','已进入隔离测试后续流程','QUALITY_TEST_AFTERSALES_001',
        ?,?,1,?,?,1,?,?)
      ON CONFLICT(source_key) DO NOTHING
    `).run(
      sourceKey,
      ...
      JSON.stringify({
        handlingRecommendation: "QUALITY_ANOMALY",
        categoryDisplayName: "影像与光学／无图像或影像中断",
        riskLevel: "MEDIUM",
        conclusion: `经人工复核，${item.purpose}`,
      }),
      item.id,
      occurredAt,
      occurredAt,
    ...
    db.prepare(`
      ...
      ON CONFLICT(id) DO NOTHING
    `).run(`source-link:${item.id}`, item.id, sourceKey, JSON.stringify({
      反馈单号: `TEST-${item.eventNo}`, 反馈人: "测试反馈人", 设备型号: "测试设备型号",
      设备序列号: "TEST-SN-001", 问题描述: item.purpose,
    }), occurredAt);
    if (item.analysis) {
      const analysisContent = {
        problemDirection: "隔离测试问题原因核验",
        confirmedCategoryReference: "影像与光学／无图像或影像中断",
        sourceFactSummary: [item.purpose, "本事件只使用虚拟测试身份和测试数据"],
        confirmedFacts: ["测试事件已完成正式通报", "测试通知不得发送至真实钉钉账号"],
        analysisBasis: ["依据隔离测试来源事实和主管最终研判"],
        preliminaryConclusion: "建议由测试主管组织本部门测试员工完成原因核验。",
        causeHypotheses: ["测试流程中的模拟原因待验证"],
        investigationDirections: ["复核测试记录", "形成原因和措施证据"],
        informationGaps: [],
        handlingRequirements: ["完成原因核查", "上传验证证据", "逐级完成验收"],
        suggestedTotalDueAt: dueAt,
      };
      const analysisDeliverables = [{
        deliverableId: `quality-test-deliverable:${item.eventNo}:1`,
        name: "原因排查与验证记录",
        description: "形成可复核的测试原因排查、措施与验证记录。",
        acceptanceCriteria: "包含事实依据、原因结论、处理措施和验证结果。",
        source: "HUMAN_CUSTOM",
        selected: true,
      ...
      db.prepare(`
        ...
        ) VALUES(?,?,1,?,NULL,?,?,'{}','隔离测试正式初析','quality-test-department',
          ?,'[]',?,?,'active',?,'quality-analysis-output-v1',NULL,NULL,NULL,
          'quality-analysis-rules-v1','quality-test-case-library',
          'quality-product-knowledge-v1',NULL,'QUALITY_TEST_SPECIALIST_001',
          'QUALITY_TEST_SPECIALIST_001',?,?)
        ON CONFLICT(analysis_id) DO NOTHING
      `).run(
        `analysis:${item.id}`,
        item.id,
        `quality-test-analysis-request-${item.eventNo}`,
