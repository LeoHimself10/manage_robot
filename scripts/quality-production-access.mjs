...
export function createProductionAccess({secret, userId, origin}) {
  ...
  return function authenticate(req, now = Date.now()) {
    ...
    try {
      ...
      if (session.userId !== userId || session.loginSource !== 'dingtalk_authcode' || session.impersonation) return null;
      if (session.dingUser?.userId !== userId || !Number.isFinite(session.exp) || session.exp * 1000 <= now) return null;
      ...
      return Object.freeze({userId, session});
    } catch { return null; }
  };
}
