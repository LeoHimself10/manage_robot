...
export async function createOaHandler({root,runtime,names={},workflowFactory}) {
  const dataRoot=join(root,'data/quality-oa'); await mkdir(dataRoot,{recursive:true});
  ...
  try {install(JSON.parse(await readFile(configFile,'utf8')));}catch(e){if(e.code!=='ENOENT')throw e;}
  ...
  return {
    ...
    async handle(req,res){
      ...
      if(req.headers.origin&&req.headers.origin!=='http://'+host){json(res,403,{ok:false,error:'不允许跨站请求'});return true;}
      if(req.headers['sec-fetch-site']==='cross-site'){json(res,403,{ok:false,error:'请从本机入口打开'});return true;}
      if(url.pathname==='/oa/login'&&req.method==='GET'&&same(url.searchParams.get('key'),bootstrap)){
