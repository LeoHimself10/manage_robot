# Sync local progress-digest code to ECS, enable PROGRESS_DIGEST_* env, rebuild dingtalk container.
#
#   .\scripts\ecs-deploy-progress-digest.ps1 -PublicIp 47.243.199.153 -PemPath "$env:USERPROFILE\Downloads\hh.pem"
#
param(
...
set_kv PROGRESS_DIGEST_LLM_ENABLED 1
...
echo "--- PROGRESS_DIGEST env in container ---"
docker exec manage-robot-dingtalk sh -c 'echo PROGRESS_DIGEST_ENABLED=$PROGRESS_DIGEST_ENABLED; echo WORKBENCH_DINGTALK_NOTIFY_ENABLED=$WORKBENCH_DINGTALK_NOTIFY_ENABLED' 2>/dev/null || true
...
Write-Host "Done. Progress digest runs weekdays 9:00 Asia/Shanghai (scan every 5min)."
