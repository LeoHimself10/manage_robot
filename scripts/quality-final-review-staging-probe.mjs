...
db.prepare("UPDATE quality_assignment_nodes SET status='IN_PROGRESS' WHERE event_id=? AND parent_node_id IS NULL").run(row.event_id);
...
async function tong(){const res=await request('/api/quality-oa/tong','');const data=await res.json();assert.equal(res.status,200,JSON.stringify(data));const item=data.items.find(i=>i.workspace.event.eventId===row.event_id);assert.ok(item);return item;}
async function finalAction(action,extra={}){const item=await tong();return post('/api/quality-oa/tong/'+action,{id:item.id,expectedVersion:item.finalReview.event.version,requestId:randomUUID(),...extra},'');}
async function resubmitReturnedEmployee(){
 let w=await work();await upload(undefined,w.files.find(f=>f.current&&f.requirementId)?.evidenceId);w=await work();
 await post(node+'/submit-completion',{expectedVersion:w.nodeVersion,completionNote:'终验退回后已补充复测',requirementRevision:w.requirementRevision,requestId:randomUUID()});
 w=await work();await post(reviewPath,{subtaskId:row.subtask_id,decision:'APPROVE',reason:'复测证据核验通过',expectedVersion:w.nodeVersion,requestId:randomUUID()},'manager');
 assert.equal((await tong()).finalReview.event.status,'PENDING_QUALITY_REVIEW');
}
const ready=await tong();assert.equal(ready.finalReview.event.status,'PENDING_QUALITY_REVIEW');assert.ok(ready.finalReview.nodes.some(n=>n.work?.files.length));
const root=ready.finalReview.nodes.find(n=>n.isPrimary);assert.ok(root);
const previousComments=ready.finalReview.comments.length;
const returnEmployee={id:ready.id,expectedVersion:ready.finalReview.event.version,requestId:randomUUID(),opinion:'补充终验验证记录',nodeId:row.node_id,targetKind:'EMPLOYEE'};
assert.equal((await request('/api/quality-oa/tong/final-return','employee-1',returnEmployee)).status,403);
assert.equal((await request('/api/quality-oa/tong/final-return','',{...returnEmployee,opinion:''})).status,409);
assert.equal((await request('/api/quality-oa/tong/final-return','',{...returnEmployee,targetKind:'MANAGER'})).status,409);
assert.equal((await request('/api/quality-oa/tong/final-return','',{...returnEmployee,expectedVersion:1})).status,409);
const siblingSnapshot=JSON.stringify(db.prepare("SELECT n.node_id,n.status,s.status AS formal_status FROM quality_assignment_nodes n JOIN quality_task_links l ON l.node_id=n.node_id JOIN subtasks s ON s.subtask_id=l.subtask_id WHERE n.event_id=? AND n.node_id<>? ORDER BY n.node_id").all(row.event_id,row.node_id));
await post('/api/quality-oa/tong/final-return',returnEmployee,'');await post('/api/quality-oa/tong/final-return',returnEmployee,'');
assert.equal((await work()).nodeStatus,'RETURNED');assert.equal((await tong()).finalReview.comments.length,previousComments);
assert.equal(JSON.stringify(db.prepare("SELECT n.node_id,n.status,s.status AS formal_status FROM quality_assignment_nodes n JOIN quality_task_links l ON l.node_id=n.node_id JOIN subtasks s ON s.subtask_id=l.subtask_id WHERE n.event_id=? AND n.node_id<>? ORDER BY n.node_id").all(row.event_id,row.node_id)),siblingSnapshot);
await resubmitReturnedEmployee();
console.log('PASS Tong employee return: permission, target type, version, reason, idempotency, sibling preservation, employee evidence -> manager re-review -> final');
await finalAction('final-return',{nodeId:root.nodeId,targetKind:'MANAGER',opinion:'请主管补充整体说明'});
assert.equal((await work()).nodeStatus,'APPROVED');const managerReturned=await detail('manager');assert.equal(managerReturned.managerReturns.length,1);assert.equal(managerReturned.managerReturns[0].reason,'请主管补充整体说明');
assert.deepEqual(managerReturned.event.managerStages,['REVIEW']);
const handleBody={expectedVersion:managerReturned.event.version,reason:'已核实整体处理要求，说明齐全',requestId:randomUUID()};
assert.equal((await request('/api/workbench/quality/nodes/'+root.nodeId+'/manager-return','employee-1',handleBody)).status,403);
await post('/api/workbench/quality/nodes/'+root.nodeId+'/manager-return',handleBody,'manager');await post('/api/workbench/quality/nodes/'+root.nodeId+'/manager-return',handleBody,'manager');
assert.equal((await tong()).finalReview.event.status,'PENDING_QUALITY_REVIEW');
await finalAction('final-return',{nodeId:root.nodeId,targetKind:'MANAGER',opinion:'请主管组织补充复测'});
await post('/api/workbench/quality/nodes/'+root.nodeId+'/manager-return',{expectedVersion:(await tong()).finalReview.event.version,childNodeId:row.node_id,reason:'指定员工补充复测证据',requestId:randomUUID()},'manager');
await resubmitReturnedEmployee();assert.equal((await tong()).finalReview.comments.length,previousComments);
console.log('PASS manager return: approvals retained, visible manager action, response-only handoff and scoped employee supplementation, no OA comment enqueued');
const beforeClose=await tong(),closeBody={id:beforeClose.id,expectedVersion:beforeClose.finalReview.event.version,requestId:randomUUID(),opinion:'隔离回归：证据齐全，终验通过',processInstanceId:'malicious-browser-target'};
await post('/api/quality-oa/tong/final-close',closeBody,'');await post('/api/quality-oa/tong/final-close',closeBody,'');
let closed=await tong();assert.equal(closed.finalReview.event.status,'CLOSED');assert.equal(closed.finalReview.comments.length,previousComments+1);assert.equal(closed.finalReview.comments[0].status,'SUPPRESSED');
const queued=db.prepare('SELECT * FROM quality_final_comment_outbox WHERE closure_id=?').get(closeBody.requestId);assert.equal(queued.process_instance_id,closed.source.instanceId);assert.equal(queued.comment_user_id,userId);assert.equal(queued.attempts,0);
assert.equal((await work()).canEdit,false);const readonlyManager=await detail('manager');assert.ok(readonlyManager.audit.some(a=>a.reason===closeBody.opinion));
await post('/api/quality-oa/tong/comment-retry',{id:closed.id,closureId:closeBody.requestId},'');assert.equal(db.prepare('SELECT attempts FROM quality_final_comment_outbox WHERE closure_id=?').get(closeBody.requestId).attempts,0);
await finalAction('final-reopen',{nodeId:row.node_id,targetKind:'EMPLOYEE',opinion:'隔离回归：重开节点复核'});await resubmitReturnedEmployee();
console.log('PASS close and reopen: final opinion only, frozen server-side OA binding, test suppression, duplicate close protection, manager result visibility, historical evidence retained');
console.log('VISUAL_EVENT',ready.id,row.event_id);
